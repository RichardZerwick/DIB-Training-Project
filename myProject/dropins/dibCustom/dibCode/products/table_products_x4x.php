<?php 
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "product_name"=>PDO::PARAM_STR, "product_category"=>PDO::PARAM_STR, "product_quantity"=>PDO::PARAM_INT, "createdAt"=>PDO::PARAM_STR, "updatedAt"=>PDO::PARAM_STR, "createdBy"=>PDO::PARAM_INT, "price"=>PDO::PARAM_STR, "totalValue2"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "product_name"=>'none', "product_category"=>'none', "product_quantity"=>'none', "createdAt"=>'none', "updatedAt"=>'none', "createdBy"=>'none', "price"=>'none', "totalValue2"=>'dibsqli');
    // List of items based on 'sql insert' expresions
    public $sqlFields = array('totalValue2'=>"product_quantity * price");
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container products specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        ...
        if($getFirstOnly) { // Used after deletes to navigate to first record
            $sql = "SELECT `products`.`id` FROM $fromClause $criteria ORDER BY `products`.`id` limit 1";
        ...
        $sql = "SELECT count(*) as dib__total, min(`products`.`id`) as dib__minid, max(`products`.`id`) as dib__maxid FROM $fromClause $criteria";
        $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
        if(empty($rst))
        ...
        if($totalCount>0) {
            ...
            $tempCrit = ($criteria === '') ? "WHERE `products`.`id` < :pk1 $permsCrit" : "$criteria AND `products`.`id` < :pk1";
            $sql = "SELECT max(`products`.`id`) as dib__id, count(`products`.`id`) as dib__counter FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
            ...
            // Get next
            $tempCrit = ($criteria === '') ? "WHERE `products`.`id` > :pk1 $permsCrit" : "$criteria AND `products`.`id` > :pk1";
            $sql = "SELECT min(`products`.`id`) as dib__id FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`products`.`id` = :pk1 ";
                $sql = "SELECT `products`.`id`,`products`.`product_name`,`products`.`product_category`,`products`.`product_quantity`,`products`.`createdAt`,`products`.`updatedAt`,`products`.`createdBy`,`products`.`price` 
            , product_quantity * price AS `totalValue2`
                         FROM `products` 
                        ";
                $sql .= "WHERE $criteria";
                ...
                if($page === 1 || $countMode==='all'){
                    $sql = "SELECT Count(*) AS totalcount FROM `products`   " . (!empty($criteria) ? 'WHERE ' . substr($criteria, 4) : '');
                ...
                if ($criteria !== '') {
                    ...
                    if($usedSqlField || !empty($activeFilter)) {
						$join = "
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
                        $sql = "SELECT Count(*) AS filteredcount FROM `products` $join   $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`products`.`' . $record['property'] . '` ' . $direction . ', ';
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `products`.`id` AS `Id`, `products`.`product_name` AS `Product Name`, `products`.`product_category` AS `Product Category`, `products`.`product_quantity` AS `Product Quantity`, `products`.`createdAt` AS `CreatedAt`, `products`.`updatedAt` AS `UpdatedAt`, `products`.`createdBy` AS `CreatedBy`, `products`.`price` AS `Price` 
            , product_quantity * price AS `total value sql`
            FROM `products` 
                 ";
else
    $sql = "SELECT `products`.`id`,`products`.`product_name`,`products`.`product_category`,`products`.`product_quantity`,`products`.`createdAt`,`products`.`updatedAt`,`products`.`createdBy`,`products`.`price` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
            ...
            DibApp::load('setNgxMaterial/warehouse', 'Test.php', 'components');
$t = new Test();
$result = $t->afterReadMany('products', $attributes, 'after readmany', $filterParams);
            if(isset($result[0]) && $result[0]==='cancel') return array('error', $result[1]);
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(isset($attributes['product_name']) && trim((string)($attributes['product_name'])) !== '') {
                if(strlen($attributes["product_name"]) > 255)
                    return array ('error',"The 'Product Name' field cannot contain more than 255 characters");
            }  else
                return array ('error',"The 'Product Name' field is required. Please provide a value.");
            if(empty($attributes['product_category'])) $attributes['product_category'] = null; // change '' to null for enum types
	        // product_category (plain text)
            if(isset($attributes['product_category']) && trim((string)($attributes['product_category'])) !== '') {
                if(strlen($attributes["product_category"]) > 255)
                    return array ('error',"The 'Product Category' field cannot contain more than 255 characters");
            }  else
                return array ('error',"The 'Product Category' field is required. Please provide a value.");
	        // product_quantity (integer)
            if(isset($attributes['product_quantity']) && trim((string)($attributes['product_quantity'])) !== '') {
                if(!is_int((int)$attributes["product_quantity"]) || !ctype_digit((string)abs((int)$attributes["product_quantity"])))
                    return array ('error',"The 'Product Quantity' field must be an integer value.");
                if($attributes["product_quantity"] < -2147483647)
                    return array ('error',"The 'Product Quantity' field value must be greater than or equal to -2147483647");
                if($attributes["product_quantity"] > 2147483647)
                    return array ('error',"The 'Product Quantity' field value must be less than or equal to 2147483647");
            }  else
                return array ('error',"The 'Product Quantity' field is required. Please provide a value.");
	        // createdAt (datetime)
            if(isset($attributes['createdAt']) && trim((string)($attributes['createdAt'])) !== '') {
                if(!strtotime(str_replace('/', '-', $attributes["createdAt"])))
                    return array ('error',"The 'CreatedAt' field must be a valid date-time.");
                else
                    $attributes["createdAt"] = date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $attributes["createdAt"])));
            }  else
                return array ('error',"The 'CreatedAt' field is required. Please provide a value.");
	        // updatedAt (datetime)
            if(isset($attributes['updatedAt']) && trim((string)($attributes['updatedAt'])) !== '') {
                if(!strtotime(str_replace('/', '-', $attributes["updatedAt"])))
                    return array ('error',"The 'UpdatedAt' field must be a valid date-time.");
                else
                    $attributes["updatedAt"] = date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $attributes["updatedAt"])));
            }  else
                return array ('error',"The 'UpdatedAt' field is required. Please provide a value.");
	        // createdBy (integer)
            if(isset($attributes['createdBy']) && trim((string)($attributes['createdBy'])) !== '') {
                if(!is_int((int)$attributes["createdBy"]) || !ctype_digit((string)abs((int)$attributes["createdBy"])))
                    return array ('error',"The 'CreatedBy' field must be an integer value.");
                if($attributes["createdBy"] < -2147483647)
                    return array ('error',"The 'CreatedBy' field value must be greater than or equal to -2147483647");
                if($attributes["createdBy"] > 2147483647)
                    return array ('error',"The 'CreatedBy' field value must be less than or equal to 2147483647");
            }  else
                return array ('error',"The 'CreatedBy' field is required. Please provide a value.");
	        // price (decimal)
            if(isset($attributes['price']) && trim((string)($attributes['price'])) !== '') {
                if(!is_numeric($attributes["price"]))
                    return array ('error',"The 'Price' field must be a valid integer or decimal value.");
                $d = (string)(double)$attributes["price"];
                if(is_infinite($d) || is_nan($d))
                    return array ('error',"The 'Price' field must be a valid integer or decimal value.");
                if($attributes["price"] < -999999)
                    return array ('error',"The 'Price' field value must be greater than or equal to -999999");
                if($attributes["price"] > 999999)
                    return array ('error',"The 'Price' field value must be less than or equal to 999999");
            }               
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue) {
                    ...
                    $this->auditInsert("create", $fieldName, null, $newValue, 155, $recordId, $newDisplayValue, $entityId);
                }
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            $params[":pk1"] = $pkValues["id"];
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            $pkCrit = "`products`.`id` = :pk1";
            // Check Unique Values for table option PRIMARY(550)
            $criteria = '1=1 ';           
            if(!array_key_exists('id', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `id` FROM `products` WHERE $pkCrit", $this->dbIndex, $params, true);
                $attributes['id'] = $uRst['id'];
            }
            $criteria .= " AND `id` = :fk1 ";
            $criteria .= "AND `id` <> :pk1";
            $sql = "SELECT `products`.`id` AS pkv FROM `products` WHERE $criteria";
                    ...
                    if ($crit===TRUE) {
                        ...
                        foreach ($validAttributes AS $fieldName => $newValue) {
                            if ($newValue !== $recordOld[$fieldName]) {
                                ...
                                $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 155, $recordId, $newDisplayValue, $entityId, $oldDisplayValue);
                            }
                        }
                    } elseif (is_array($crit)) return $crit;
                }
    ...
    public function delete($pkValues, $clientData=array()) {
        try {
            ...
            $sql = "DELETE `products` FROM `products`   WHERE $crit";
            dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) {
                        ...
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 155, $recordId, null, $entityId, $oldDisplayValue);
                    }
                } elseif (is_array($crit)) return $crit;
                return true;
            }
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `product_name`, `product_category`, `product_quantity`, `createdAt`, `updatedAt`, `createdBy`, `price`
                    FROM `products` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for products.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container products records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'products' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'products' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = "UPDATE products SET `" . $args[0] . "` = :value WHERE $pkCrit";					
					DibApp::$array['DuplicateRecords']['products*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 365, 'products', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Product Name", "Product Category", "Product Quantity", "CreatedAt", "UpdatedAt", "CreatedBy", "Price", "total value sql" );
    }
    ...
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('products', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx4xxproducts",
					 'selectFields' => "`products`.`id`,`products`.`product_name`,`products`.`product_category`,`products`.`product_quantity`,`products`.`createdAt`,`products`.`updatedAt`,`products`.`createdBy`,`products`.`price`",
            ...
            ", ", \r\n"),
                     ...
                     'from' => trim("`products` /*_dib DROPDOWNS - SQL DISPLAY EXPR dib_*/ 
