<?php 
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "default_url"=>PDO::PARAM_STR, "login_group_expiry"=>PDO::PARAM_STR, "username"=>PDO::PARAM_STR, "pef_security_policy_id"=>PDO::PARAM_INT, "last_login_datetime"=>PDO::PARAM_STR, "login_attempts"=>PDO::PARAM_INT, "password_history"=>PDO::PARAM_STR, "reset_expiry"=>PDO::PARAM_STR, "language"=>PDO::PARAM_STR, "first_name"=>PDO::PARAM_STR, "login_lockout"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "last_name"=>PDO::PARAM_STR, "login_blocked_count"=>PDO::PARAM_BOOL, "mobile_number"=>PDO::PARAM_STR, "password"=>PDO::PARAM_STR, "login_expiry"=>PDO::PARAM_STR, "larger_font"=>PDO::PARAM_BOOL, "test_user"=>PDO::PARAM_BOOL, "password_expiry"=>PDO::PARAM_STR, "password_remember_expiry"=>PDO::PARAM_STR, "password_remember"=>PDO::PARAM_STR, "password_remember_hash"=>PDO::PARAM_STR, "dib_username"=>PDO::PARAM_STR, "supplier_code"=>PDO::PARAM_STR, "created_datetime"=>PDO::PARAM_STR, "reset_guid"=>PDO::PARAM_STR, "activation_guid"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "dibuid"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "default_url"=>'none', "login_group_expiry"=>'none', "username"=>'none', "pef_security_policy_id"=>'none', "last_login_datetime"=>'none', "login_attempts"=>'none', "password_history"=>'none', "reset_expiry"=>'none', "language"=>'none', "first_name"=>'none', "login_lockout"=>'none', "email"=>'none', "last_name"=>'none', "login_blocked_count"=>'none', "mobile_number"=>'none', "password"=>'none', "login_expiry"=>'none', "larger_font"=>'none', "test_user"=>'none', "password_expiry"=>'none', "password_remember_expiry"=>'none', "password_remember"=>'none', "password_remember_hash"=>'none', "dib_username"=>'none', "supplier_code"=>'none', "created_datetime"=>'none', "reset_guid"=>'none', "activation_guid"=>'none', "notes"=>'none', "dibuid"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'profile') === 0 || strpos($activeFilter, '___profile') === (strlen($activeFilter)-strlen("___profile"))) {
            $crit = "pef_login.id = :dibUser_id"; 
                    if(array_key_exists('id', DIB::$USER)) {
                        $params[':dibUser_id']=DIB::$USER['id'];
                        if(array_key_exists('dibUser_id', $filterParams)) unset($filterParams['dibUser_id']);
                    }
            $criteria .= " AND ($crit) ";
        }
        if($criteria==='') {
            Log::err("The read request for container profile specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(!array_key_exists('dib_username', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `dib_username` is in the design of container with id 371. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 556 are submitted to the server (ie they exist as (hidden) fields in container id 371).\r\n".
            ...
            if(!array_key_exists('email', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `email` is in the design of container with id 371. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 557 are submitted to the server (ie they exist as (hidden) fields in container id 371).\r\n".
                         "NOTE: if one of these fields is a dropdown or other component that has a port that points to a container which the current permission group does NOT have access to, this component will not be included. Simply remove the link to the Port record and regenerate CRUD file.");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
            ...
            if(!array_key_exists('username', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `username` is in the design of container with id 371. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 561 are submitted to the server (ie they exist as (hidden) fields in container id 371).\r\n".
                         "NOTE: if one of these fields is a dropdown or other component that has a port that points to a container which the current permission group does NOT have access to, this component will not be included. Simply remove the link to the Port record and regenerate CRUD file.");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            // login_blocked_count (boolean)
        	$attributes["login_blocked_count"] = (in_array($attributes["login_blocked_count"], array(1, True, '1', 'True'), TRUE)) ? 1 : 0;            	
            // login_expiry (datetime)
            if(isset($attributes['login_expiry']) && trim((string)($attributes['login_expiry'])) !== '') {
                if(!strtotime(str_replace('/', '-', $attributes["login_expiry"])))
                    return array ('error',"The 'Login Expiry' field must be a valid date-time.");
            ...
            // larger_font (boolean)
        	$attributes["larger_font"] = (in_array($attributes["larger_font"], array(1, True, '1', 'True'), TRUE)) ? 1 : 0;            	
            // test_user (boolean)
        	$attributes["test_user"] = (in_array($attributes["test_user"], array(1, True, '1', 'True'), TRUE)) ? 1 : 0;            	
            // password_expiry (datetime)
            if(isset($attributes['password_expiry']) && trim((string)($attributes['password_expiry'])) !== '') {
                if(!strtotime(str_replace('/', '-', $attributes["password_expiry"])))
                    return array ('error',"The 'Password Expiry' field must be a valid date-time.");
            ...
            // Check Unique Values for table option PRIMARY(554)
            $criteria = '1=1 ';           
            if(!array_key_exists('id', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `id` FROM `pef_login` WHERE $pkCrit", $this->dbIndex, $params, true);
            ...
            // Check Unique Values for table option dib_username(556)
            $criteria = '1=1 ';           
            if(!array_key_exists('dib_username', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `dib_username` FROM `pef_login` WHERE $pkCrit", $this->dbIndex, $params, true);
                $attributes['dib_username'] = $uRst['dib_username'];
            }
            $criteria .= " AND `dib_username` = :fk1 ";
            $criteria .= "AND `id` <> :pk1";
            $sql = "SELECT `pef_login`.`id` AS pkv FROM `pef_login` WHERE $criteria";
            ...
            if(!empty($rst)) {
                if(count($paramsU) > 1)
                    return array('error',"Update record cancelled. The combination of values in 'dib_username' needs to be unique. Another record already contains the same combination of values.");
                else
                    return array('error',"Update record cancelled. The value in 'dib_username' needs to be unique. Another record already contains the same value.");
            }
            // Check Unique Values for table option email(557)
            $criteria = '1=1 ';           
            if(!array_key_exists('email', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `email` FROM `pef_login` WHERE $pkCrit", $this->dbIndex, $params, true);
            ...
            // Check Unique Values for table option username_UNIQUE(561)
            $criteria = '1=1 ';           
            if(!array_key_exists('username', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `username` FROM `pef_login` WHERE $pkCrit", $this->dbIndex, $params, true);
                $attributes['username'] = $uRst['username'];
            }
            $criteria .= " AND `username` = :fk1 ";
            $criteria .= "AND `id` <> :pk1";
            $sql = "SELECT `pef_login`.`id` AS pkv FROM `pef_login` WHERE $criteria";
            ...
            if(!empty($rst)) {
                if(count($paramsU) > 1)
                    return array('error',"Update record cancelled. The combination of values in 'username' needs to be unique. Another record already contains the same combination of values.");
                else
                    return array('error',"Update record cancelled. The value in 'username' needs to be unique. Another record already contains the same value.");
            }
            $crit = $pkCrit;

            // Get record's existing (old) values
            $sql = "SELECT `pef_login`.* 
		            FROM `pef_login`
                        WHERE $crit";
                    ...
                    if ($crit===TRUE) {
                        ...
                        foreach ($validAttributes AS $fieldName => $newValue) {
                            if ($newValue !== $recordOld[$fieldName]) {
                                ...
                                $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 157, $recordId, $newDisplayValue, $entityId, $oldDisplayValue);
                            }
                        }
                    } elseif (is_array($crit)) return $crit;
                }
    ...
    public function delete($pkValues, $clientData=array()) {
        try {
            ...
            $sql = "DELETE `pef_login` FROM `pef_login`   WHERE $crit";
            dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) {
                        ...
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 157, $recordId, null, $entityId, $oldDisplayValue);
                    }
                } elseif (is_array($crit)) return $crit;
                return true;
            }
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `username`, `first_name`, `last_name`, `password`, `default_url`, `pef_security_policy_id`, `login_attempts`, `reset_expiry`, `login_lockout`, `login_blocked_count`, `login_expiry`, `login_group_expiry`, `last_login_datetime`, `password_history`, `language`, `email`, `mobile_number`, `larger_font`, `test_user`, `password_expiry`, `password_remember_expiry`, `password_remember`, `password_remember_hash`, `dib_username`, `supplier_code`, `created_datetime`, `reset_guid`, `activation_guid`, `notes`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for profile.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container profile records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'profile' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'profile' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 371, 'pef_login', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Default Url", "Login Group Expiry", "Username", "Pef Security Policy", "Last Login Datetime", "Login Attempts", "Password History", "Reset Expiry", "Language", "First Name", "Login Lockout", "Email", "Last Name", "Login Blocked Count", "Mobile Number", "Password", "Login Expiry", "Larger Font", "Test User", "Password Expiry", "Password Remember Expiry", "Password Remember", "Password Remember Hash", "Dib Username", "Supplier Code", "Created Datetime", "Reset Guid", "Activation Guid", "Notes", "Dibuid" );
    }
    ...
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('profile', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx4xxprofile",
            ...
            ", ", \r\n"),
             ...
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/  /*_dib EXTRA-TABLE-JOIN dib_*/ ", ", \r\n"),
                     'order_by' => "",
                     'filter' => "pef_login.id = :dibUser_id"
        );
	}
} // end Class
