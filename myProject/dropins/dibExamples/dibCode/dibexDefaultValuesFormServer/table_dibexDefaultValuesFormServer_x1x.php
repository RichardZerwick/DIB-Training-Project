<?php 
    Class table_dibexDefaultValuesFormServer_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "updated"=>PDO::PARAM_STR, "client_id"=>PDO::PARAM_INT, "notes"=>PDO::PARAM_STR, "first_name"=>PDO::PARAM_STR, "last_name"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "updated"=>'none', "client_id"=>'dropdown', "notes"=>'none', "first_name"=>'none', "last_name"=>'none');
    // List of items based on 'sql insert' expresions
    public $sqlFields = array();
    public $fkeyDisplay = array('client_id'=>"`test_client1000`.`name`",
            );
    ...
    private function parseFilterArray($filterParams=array(), &$params=array(), &$fieldType=array()) {
        ...
        foreach($filterParams as $record) {
            ...
            if(array_key_exists($field, $this->sqlFields))
            	...
            	$str .= '`test_client_contact`.`' . $field . '` = :pk' . $i . ' AND ';
            $params[':pk' . $i] = $value;
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexDefaultValuesFormServer specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        ...
        if(!empty($activeFilter)) {
            ...
            $fromClause = "`test_client_contact`
                /*_dib client_id (6207) dib_*/
				LEFT JOIN `test_client` `test_client1000` ON 
                    `test_client_contact`.`client_id` = `test_client1000`.`id` 
                ";
        } else  
            $fromClause = "`test_client_contact`  ";
        $criteria .= $permsCrit;
        ...
        if($getFirstOnly) { // Used after deletes to navigate to first record
            $sql = "SELECT `test_client_contact`.`id` FROM $fromClause $criteria ORDER BY `test_client_contact`.`id` limit 1";
        ...
        $sql = "SELECT count(*) as dib__total, min(`test_client_contact`.`id`) as dib__minid, max(`test_client_contact`.`id`) as dib__maxid FROM $fromClause $criteria";
        $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
        if(empty($rst))
        ...
        if($totalCount>0) {
            ...
            $tempCrit = ($criteria === '') ? "WHERE `test_client_contact`.`id` < :pk1 $permsCrit" : "$criteria AND `test_client_contact`.`id` < :pk1";
            $sql = "SELECT max(`test_client_contact`.`id`) as dib__id, count(`test_client_contact`.`id`) as dib__counter FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
            ...
            // Get next
            $tempCrit = ($criteria === '') ? "WHERE `test_client_contact`.`id` > :pk1 $permsCrit" : "$criteria AND `test_client_contact`.`id` > :pk1";
            $sql = "SELECT min(`test_client_contact`.`id`) as dib__id FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
...
$sql = "SELECT `test_client_contact`.`id` 
        FROM $fromClause
        $criteria
        ORDER BY `test_client_contact`.`id` 
        LIMIT " . ($position - 1) . ', 1';
        $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
        if(empty($rst))
            return null;
    ...
    public function parseGridFilter($gridFilter, &$params=array(), &$fieldType=array(), &$usedSqlField=FALSE, &$i = 0) {
        ...
        foreach($gridFilter as $row) {
            ...
            if(array_key_exists($field, $this->sqlFields)) {
            	...
            	$fieldExpr = "`test_client_contact`.`$field`";
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`test_client_contact`.`id` = :pk1 ";
                $sql = "SELECT `test_client_contact`.`id`,`test_client_contact`.`client_id`,`test_client_contact`.`first_name`,`test_client_contact`.`last_name`,`test_client_contact`.`notes`,`test_client_contact`.`updated` 
                ...
                if($page === 1 || $countMode==='all'){
                    $sql = "SELECT Count(*) AS totalcount FROM `test_client_contact`   " . (!empty($criteria) ? 'WHERE ' . substr($criteria, 4) : '');
				...
				LEFT JOIN `test_client` `test_client1000` ON 
                    `test_client_contact`.`client_id` = `test_client1000`.`id`";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
                        $sql = "SELECT Count(*) AS filteredcount FROM `test_client_contact` $join   $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`test_client_contact`.`' . $record['property'] . '` ' . $direction . ', ';
                    }
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_client_contact`.`id` AS `Id`, `test_client_contact`.`first_name` AS `First Name`, `test_client_contact`.`last_name` AS `Last Name`, `test_client_contact`.`updated` AS `Updated`, `test_client_contact`.`notes` AS `Notes` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
        ...
        try { 
            ...
            if(isset($attributes['first_name']) && trim((string)($attributes['first_name'])) !== '') {
                if(strlen($attributes["first_name"]) > 35)
                    return array ('error',"The 'First Name' field cannot contain more than 35 characters");
            }  else
            ...
            if(isset($attributes['last_name']) && trim((string)($attributes['last_name'])) !== '') {
                if(strlen($attributes["last_name"]) > 35)
                    return array ('error',"The 'Last Name' field cannot contain more than 35 characters");
            }  else
            ...
            if(isset($attributes['notes']) && trim((string)($attributes['notes'])) !== '') {
                if(strlen($attributes["notes"]) > 250)
                    return array ('error',"The 'Notes' field cannot contain more than 250 characters");
            }               
	        // client_id (integer)
            if(isset($attributes['client_id']) && trim((string)($attributes['client_id'])) !== '') {
                if(!is_int((int)$attributes["client_id"]) || !ctype_digit((string)abs((int)$attributes["client_id"])))
                    return array ('error',"The 'Client' field must be an integer value.");
                if($attributes["client_id"] < -2147483647)
                    return array ('error',"The 'Client' field value must be greater than or equal to -2147483647");
                if($attributes["client_id"] > 2147483647)
                    return array ('error',"The 'Client' field value must be less than or equal to 2147483647");
            }  else
                return array ('error',"The 'Client' field is required. Please provide a value.");
	        // updated (datetime)
            ...
            $sql = "INSERT INTO `test_client_contact` (";
            $fieldList = '';
            $valueList = '';
            $fieldType = array();
            $i=0;
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue) {
                    ...
                    $this->auditInsert("create", $fieldName, null, $newValue, 67, $recordId, $newDisplayValue, $entityId);
                }
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            $params[":pk1"] = $pkValues["id"];
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            $pkCrit = "`test_client_contact`.`id` = :pk1";
            // Check Unique Values for table option PRIMARY(441)
            $criteria = '1=1 ';           
            if(!array_key_exists('id', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `id` FROM `test_client_contact` WHERE $pkCrit", $this->dbIndex, $params, true);
                $attributes['id'] = $uRst['id'];
            }
            $criteria .= " AND `id` = :fk1 ";
            $criteria .= "AND `id` <> :pk1";
            $sql = "SELECT `test_client_contact`.`id` AS pkv FROM `test_client_contact` WHERE $criteria";
            ...
            $sql = "SELECT `test_client_contact`.* 
                           , `test_client1000`.`name` as `client_id_display_value`
		            FROM `test_client_contact`
                 		/*_dib client_id (6207) dib_*/
				LEFT JOIN `test_client` `test_client1000` ON 
            ...
            foreach ($validAttributes AS $key => $value) {
                if(!array_key_exists($key, $recordOld) || (string)$value === (string)$recordOld[$key] || !array_key_exists($key, $this->storeType) || $this->storeType[$key] === 'dibsqli')
                    ...
                    // Only update the fields that have changed values
                        $sql .= "`test_client_contact`.`$key`=:f$i, ";
                    $params[':f'.$i] = $value;
                    $fieldType[':f'.$i] = $this->fieldType[$key];
                    $i++;
                }
                    ...
                    if ($crit===TRUE) {
                        ...
                        foreach ($validAttributes AS $fieldName => $newValue) {
                            if ($newValue !== $recordOld[$fieldName]) {
                                ...
                                $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 67, $recordId, $newDisplayValue, $entityId, $oldDisplayValue);
                            }
                        }
                    } elseif (is_array($crit)) return $crit;
                }
    ...
    public function delete($pkValues, $clientData=array()) {
        try {
            ...
            $sql = "DELETE `test_client_contact` FROM `test_client_contact`   WHERE $crit";
            dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) {
                        ...
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 67, $recordId, null, $entityId, $oldDisplayValue);
                    }
                } elseif (is_array($crit)) return $crit;
                return true;
            }
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `client_id`, `first_name`, `last_name`, `notes`, `updated`
                    FROM `test_client_contact` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexDefaultValuesFormServer.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexDefaultValuesFormServer records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexDefaultValuesFormServer' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexDefaultValuesFormServer' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = "UPDATE test_client_contact SET `" . $args[0] . "` = :value WHERE $pkCrit";					
					DibApp::$array['DuplicateRecords']['test_client_contact*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    public function getDefaults($createParams=null, $clientData=array()) {
        try {
            $attributes=array();
            if(array_key_exists('bexDefaultValuesServer_name', DIB::$USER)) 
                $attributes['formDefaultValue'] = DIB::$USER['bexDefaultValuesServer_name']; // dibUser_ fields
            elseif(isset($clientData[trim('alias_dibexDefaultValuesServer_name')])) // See if clientData variable is referenced
                $attributes['formDefaultValue'] = $clientData[trim('alias_dibexDefaultValuesServer_name')];
            else // Try getting value from pef_param
                $attributes['formDefaultValue'] = EvalCriteria::evalParam('alias_dibexDefaultValuesServer_name', $clientData);
            $attributes['id'] = 0;
            if(array_key_exists('bexDefaultValuesServer_id', DIB::$USER)) 
                $attributes['client_id'] = DIB::$USER['bexDefaultValuesServer_id']; // dibUser_ fields
            elseif(isset($clientData[trim('alias_dibexDefaultValuesServer_id')])) // See if clientData variable is referenced
                $attributes['client_id'] = $clientData[trim('alias_dibexDefaultValuesServer_id')];
            else // Try getting value from pef_param
                $attributes['client_id'] = EvalCriteria::evalParam('alias_dibexDefaultValuesServer_id', $clientData);
            if(array_key_exists('first_name', DIB::$USER)) 
                $attributes['first_name'] = DIB::$USER['first_name']; // dibUser_ fields
            elseif(isset($clientData[trim('dibUser_first_name')])) // See if clientData variable is referenced
                $attributes['first_name'] = $clientData[trim('dibUser_first_name')];
            else // Try getting value from pef_param
                $attributes['first_name'] = EvalCriteria::evalParam('dibUser_first_name', $clientData);
            if(array_key_exists('last_name', DIB::$USER)) 
                $attributes['last_name'] = DIB::$USER['last_name']; // dibUser_ fields
            elseif(isset($clientData[trim('dibUser_last_name')])) // See if clientData variable is referenced
                $attributes['last_name'] = $clientData[trim('dibUser_last_name')];
            else // Try getting value from pef_param
                $attributes['last_name'] = EvalCriteria::evalParam('dibUser_last_name', $clientData);
            $attributes['updated'] = date('Y-m-d H:i:s');
            $attributes['notes'] = DibApp::getRealIpAddr();
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 256, 'test_client_contact', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Updated", "Client", "Notes", "First Name", "Last Name" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexDefaultValuesFormServer', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexDefaultValuesFormServer",
					 'selectFields' => "`test_client_contact`.`id`,`test_client_contact`.`client_id`,`test_client_contact`.`first_name`,`test_client_contact`.`last_name`,`test_client_contact`.`notes`,`test_client_contact`.`updated`",
				     'selectSqlFields' => trim("
            ", ", \r\n"),
				     'selectSqlDisplay' =>  trim("
            ", ", \r\n"),
				     'selectTableDisplay' => trim(", `test_client1000`.`name` as `client_id_display_value`", ", \r\n"),          
                     'from' => trim("`test_client_contact` /*_dib DROPDOWNS - SQL DISPLAY EXPR dib_*/ 
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib client_id (6207) dib_*/
				LEFT JOIN `test_client` `test_client1000` ON 
                    `test_client_contact`.`client_id` = `test_client1000`.`id` /*_dib EXTRA-TABLE-JOIN dib_*/ ", ", \r\n"),
                     'order_by' => "",
                     'filter' => ""
        );
	}
