<?php 
    Class table_dibexGridParentSqlFields_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "main_contact"=>PDO::PARAM_STR, "mobile"=>PDO::PARAM_STR, "phone_w"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "name"=>'none', "main_contact"=>'dibsqli', "mobile"=>'dibsqli', "phone_w"=>'dibsqli');
    // List of items based on 'sql insert' expresions
    public $sqlFields = array('phone_w'=>"`test_client_contact1000`.`phone_w`", 'mobile'=>"`test_client_contact1000`.`mobile`", 'main_contact'=>"CONCAT(cc.first_name, ' ', cc.last_name)");
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexGridParentSqlFields specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_client`.`id`,`test_client`.`name` 
            , CONCAT(cc.first_name, ' ', cc.last_name) AS `main_contact`
                               , `test_client_contact1000`.`phone_w` as `phone_w`
				, `test_client_contact1000`.`mobile` as `mobile`
                         FROM `test_client` 
                         /*_dib phone_w (4504) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id` 
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_client`.`id` AS `Id`, `test_client`.`name` AS `Name` 
                   , `test_client_contact1000`.`phone_w` as `Phone W (Parent Field)`
				, `test_client_contact1000`.`mobile` as `Mobile (Parent Field)` 
            , CONCAT(cc.first_name, ' ', cc.last_name) AS `Main Contact (SQL Insert Field)`
            FROM `test_client` 
                 /*_dib phone_w (4504) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `name` is in the design of container with id 180. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 438 are submitted to the server (ie they exist as (hidden) fields in container id 180).\r\n".
        ...
        try {    
            ...
            if(isset($attributes['phone_w']) && trim((string)($attributes['phone_w'])) !== '') {
                if(strlen($attributes["phone_w"]) > 35)
                    return array ('error',"The 'Phone W (Parent Field)' field cannot contain more than 35 characters");
            }               
            // name (plain text)
            if(isset($attributes['name']) && trim((string)($attributes['name'])) !== '') {
                if(strlen($attributes["name"]) > 80)
			...
			if(!is_null($attributes['main_contact_id']) && trim((string)$attributes['main_contact_id'])!=='') { // Beware... the PHP empty function catches 0,'0',false,etc.       
	            $sql = "UPDATE `test_client_contact` SET 
	            		`phone_w` = :phone_w,`mobile` = :mobile WHERE id = :id";
		        // Set parameter values
		        $updatePar = array();
		        $updatePar[':phone_w'] = $attributes['phone_w'];
		        // Check if value has changed
		        if($attributes['phone_w']!==$recordOld['phone_w'])
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexGridParentSqlFields.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexGridParentSqlFields records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexGridParentSqlFields' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexGridParentSqlFields' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 180, 'test_client', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Name", "Main Contact (SQL Insert Field)" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexGridParentSqlFields', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexGridParentSqlFields",
					 'selectFields' => "`test_client`.`id`,`test_client`.`name`",
				     'selectSqlFields' => trim("
            , CONCAT(cc.first_name, ' ', cc.last_name) AS `main_contact`
            ", ", \r\n"),
				     'selectSqlDisplay' =>  trim("
            ", ", \r\n"),
				     'selectTableDisplay' => trim(", `test_client_contact1000`.`phone_w` as `phone_w`
				, `test_client_contact1000`.`mobile` as `mobile`", ", \r\n"),          
                     'from' => trim("`test_client` /*_dib DROPDOWNS - SQL DISPLAY EXPR dib_*/ 
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib phone_w (4504) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id` /*_dib EXTRA-TABLE-JOIN dib_*/ LEFT JOIN test_client_contact cc ON test_client.main_contact_id = cc.id", ", \r\n"),
                     'order_by' => "",
                     'filter' => ""
        );
	}
