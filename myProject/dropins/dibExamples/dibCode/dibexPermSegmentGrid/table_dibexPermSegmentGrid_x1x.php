<?php 
    Class table_dibexPermSegmentGrid_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "project_id"=>PDO::PARAM_INT, "date_from"=>PDO::PARAM_STR, "date_to"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR, "perc_complete"=>PDO::PARAM_BOOL);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "project_id"=>'dropdown', "date_from"=>'none', "date_to"=>'none', "notes"=>'none', "updated"=>'none', "perc_complete"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'dibexPermSegmentDataByUser_dibexPermSegmentGrid') === 0) {
            $crit = "`test_staff_project`.`staff_id` = :alias_parent_id"; 
            if (array_key_exists('alias_parent_id',$filterParams)) {
                ...
                if(is_array($value)  || $value === ':alias_parent_id')
                    //return array('error',"Error! The filter parameter 'alias_parent_id' for filter '' on dibexPermSegmentGrid is missing from submitted values.");
        ...
        if($criteria==='') {
            Log::err("The read request for container dibexPermSegmentGrid specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        $params = array();   
        $criteria = '';
         $permsCrit = " AND ((test_staff_project.staff_id = :dibUser_staff_id))";
        if(!empty($activeFilter)) {
            ...
            $fromClause = "`test_staff_project`
                /*_dib project_id (7019) dib_*/
            LEFT JOIN test_project tp 
 ON `test_staff_project`.project_id = tp.id LEFT JOIN test_staff_project tsp ON tsp.project_id = tp.id
                ";
        } else  
            $fromClause = "`test_staff_project`  ";
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`test_staff_project`.`id` = :pk1  AND ((test_staff_project.staff_id = :dibUser_staff_id))";
                $sql = "SELECT `test_staff_project`.`id`,`test_staff_project`.`project_id`,`test_staff_project`.`date_from`,`test_staff_project`.`date_to`,`test_staff_project`.`notes`,`test_staff_project`.`updated`,`test_staff_project`.`perc_complete` 
                , tp.name AS `project_id_display_value`
...
$params[':dibUser_staff_id'] = DIB::$USER['staff_id'];
                ...
                // Permission filter
                $fieldType = array();
                $criteria = " AND ((test_staff_project.staff_id = :dibUser_staff_id))";
                $join = "";
                $params=array();
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_staff_project`.`id` AS `Id`, `test_staff_project`.`date_from` AS `Date From`, `test_staff_project`.`date_to` AS `Date To`, `test_staff_project`.`notes` AS `Notes`, `test_staff_project`.`updated` AS `Updated`, `test_staff_project`.`perc_complete` AS `Perc Complete` 
                , tp.name AS `Project`
            FROM `test_staff_project` 
                /*_dib project_id (7019) dib_*/
            LEFT JOIN test_project tp 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            // perc_complete (boolean)
        	$attributes["perc_complete"] = (in_array($attributes["perc_complete"], array(1, True, '1', 'True'), TRUE)) ? 1 : 0;            	
 ...
 ON `test_staff_project`.project_id = tp.id LEFT JOIN test_staff_project tsp ON tsp.project_id = tp.id
            ...
            $multiChanged = false; 
            $crit .= " AND ((test_staff_project.staff_id = :dibUser_staff_id AND test_staff_project.project_id IN (SELECT project_id FROM test_staff_project WHERE staff_id = :dibUser_staff_id)))";
			// Add SQL PDO parameter values in criteria
$params[':dibUser_staff_id'] = DIB::$USER['staff_id'];
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        return array('error',"Oops! The permission system restricts you from adding new records to test_staff_project.");
    }
     /**
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 308, 'test_staff_project', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Project", "Date From", "Date To", "Notes", "Updated", "Perc Complete" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexPermSegmentGrid', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexPermSegmentGrid",
					 'selectFields' => "`test_staff_project`.`id`,`test_staff_project`.`project_id`,`test_staff_project`.`date_from`,`test_staff_project`.`date_to`,`test_staff_project`.`notes`,`test_staff_project`.`updated`,`test_staff_project`.`perc_complete`",
