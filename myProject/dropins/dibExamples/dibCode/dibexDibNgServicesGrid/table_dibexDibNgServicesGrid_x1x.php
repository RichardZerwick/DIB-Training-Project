<?php 
    Class table_dibexDibNgServicesGrid_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "first_name"=>PDO::PARAM_STR, "last_name"=>PDO::PARAM_STR, "position"=>PDO::PARAM_STR, "join_date"=>PDO::PARAM_STR, "color"=>PDO::PARAM_STR, "staff_code"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "mobile"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "first_name"=>'none', "last_name"=>'none', "position"=>'none', "join_date"=>'none', "color"=>'none', "staff_code"=>'none', "email"=>'none', "mobile"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'dibexDibNgServicesGrid') === 0 || strpos($activeFilter, '___dibexDibNgServicesGrid') === (strlen($activeFilter)-strlen("___dibexDibNgServicesGrid"))) {
            $crit = "(:alias_self_positionFilter IS NULL OR test_staff.position = :alias_self_positionFilter)"; 
            if (array_key_exists('alias_self_positionFilter',$filterParams)) {
                $params[':alias_self_positionFilter'] = $filterParams['alias_self_positionFilter'];
                ...
                // unset($filterParams["alias_self_positionFilter"]);
            } else {
                $value = EvalCriteria::evalParam(':alias_self_positionFilter', $filterParams);
                if(is_array($value)  || $value === ':alias_self_positionFilter')
                    //return array('error',"Error! The filter parameter 'alias_self_positionFilter' for filter '' on dibexDibNgServicesGrid is missing from submitted values.");
                    ...
                    $params[':alias_self_positionFilter'] = $value;
            }
            $criteria .= " AND ($crit) ";
        }
        if($criteria==='') {
            Log::err("The read request for container dibexDibNgServicesGrid specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_staff`.`id`,`test_staff`.`first_name`,`test_staff`.`last_name`,`test_staff`.`position`,`test_staff`.`join_date`,`test_staff`.`color`,`test_staff`.`staff_code`,`test_staff`.`email`,`test_staff`.`mobile` 
                         FROM `test_staff` 
                        ";
                $sql .= "WHERE $criteria";
                if(strpos($sql, '{:/') !== FALSE) { 
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_staff`.`id` AS `Id`, `test_staff`.`first_name` AS `First Name`, `test_staff`.`last_name` AS `Last Name`, `test_staff`.`position` AS `Position`, `test_staff`.`join_date` AS `Join Date`, `test_staff`.`color` AS `Color`, `test_staff`.`staff_code` AS `Staff Code`, `test_staff`.`email` AS `Email`, `test_staff`.`mobile` AS `Mobile` 
            FROM `test_staff` 
                 ";
else
    $sql = "SELECT `test_staff`.`id`,`test_staff`.`first_name`,`test_staff`.`last_name`,`test_staff`.`position`,`test_staff`.`join_date`,`test_staff`.`color`,`test_staff`.`staff_code`,`test_staff`.`email`,`test_staff`.`mobile` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
	        ...
	        // staff_code (plain text)
            if(isset($attributes['staff_code']) && trim((string)($attributes['staff_code'])) !== '') {
                if(strlen($attributes["staff_code"]) > 10)
                    return array ('error',"The 'Staff Code' field cannot contain more than 10 characters");
            }  else
                return array ('error',"The 'Staff Code' field is required. Please provide a value.");
	        // email (email)
            ...
            if(!array_key_exists('email', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `email` is in the design of container with id 212. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 462 are submitted to the server (ie they exist as (hidden) fields in container id 212).\r\n".
            ...
            if(!array_key_exists('first_name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `first_name` is in the design of container with id 212. It is required since it is involved in checking the unique index\r\n".
            ...
            if(!array_key_exists('last_name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `last_name` is in the design of container with id 212. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 464 are submitted to the server (ie they exist as (hidden) fields in container id 212).\r\n".
                         "NOTE: if one of these fields is a dropdown or other component that has a port that points to a container which the current permission group does NOT have access to, this component will not be included. Simply remove the link to the Port record and regenerate CRUD file.");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
            ...
            if(!array_key_exists('staff_code', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `staff_code` is in the design of container with id 212. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 468 are submitted to the server (ie they exist as (hidden) fields in container id 212).\r\n".
                         "NOTE: if one of these fields is a dropdown or other component that has a port that points to a container which the current permission group does NOT have access to, this component will not be included. Simply remove the link to the Port record and regenerate CRUD file.");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            // Check Unique Values for table option staff_code(468)
            $criteria = '1=1 ';           
            if(!array_key_exists('staff_code', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `staff_code` FROM `test_staff` WHERE $pkCrit", $this->dbIndex, $params, true);
                $attributes['staff_code'] = $uRst['staff_code'];
            }
            $criteria .= " AND `staff_code` = :fk1 ";
            $criteria .= "AND `id` <> :pk1";
            $sql = "SELECT `test_staff`.`id` AS pkv FROM `test_staff` WHERE $criteria";
            ...
            if(!empty($rst)) {
                if(count($paramsU) > 1)
                    return array('error',"Update record cancelled. The combination of values in 'staff_code' needs to be unique. Another record already contains the same combination of values.");
                else
                    return array('error',"Update record cancelled. The value in 'staff_code' needs to be unique. Another record already contains the same value.");
            }
            $crit = $pkCrit;
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `first_name`, `last_name`, `position`, `join_date`, `color`, `staff_code`, `email`, `mobile`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexDibNgServicesGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexDibNgServicesGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexDibNgServicesGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexDibNgServicesGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 212, 'test_staff', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "First Name", "Last Name", "Position", "Join Date", "Color", "Staff Code", "Email", "Mobile" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexDibNgServicesGrid', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexDibNgServicesGrid",
					 'selectFields' => "`test_staff`.`id`,`test_staff`.`first_name`,`test_staff`.`last_name`,`test_staff`.`position`,`test_staff`.`join_date`,`test_staff`.`color`,`test_staff`.`staff_code`,`test_staff`.`email`,`test_staff`.`mobile`",
            ...
            ", ", \r\n"),
             ...
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/  /*_dib EXTRA-TABLE-JOIN dib_*/ ", ", \r\n"),
                     'order_by' => "",
                     'filter' => "(:alias_self_positionFilter IS NULL OR test_staff.position = :alias_self_positionFilter)"
        );
	}
} // end Class
