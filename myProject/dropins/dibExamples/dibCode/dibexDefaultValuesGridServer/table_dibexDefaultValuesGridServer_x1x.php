<?php 
    Class table_dibexDefaultValuesGridServer_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "client_id"=>PDO::PARAM_INT, "first_name"=>PDO::PARAM_STR, "last_name"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "client_id"=>'dropdown', "first_name"=>'none', "last_name"=>'none', "notes"=>'none', "updated"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'dibexDefaultValuesServer_dibexDefaultValuesGridServer') === 0) {
            ...
            if (array_key_exists('alias_parent_id',$filterParams)) {
                ...
                if(is_array($value)  || $value === ':alias_parent_id')
                    //return array('error',"Error! The filter parameter 'alias_parent_id' for filter '' on dibexDefaultValuesGridServer is missing from submitted values.");
        ...
        if($criteria==='') {
            Log::err("The read request for container dibexDefaultValuesGridServer specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_client_contact`.`id` AS `Id`, `test_client_contact`.`first_name` AS `First Name`, `test_client_contact`.`last_name` AS `Last Name`, `test_client_contact`.`notes` AS `Notes`, `test_client_contact`.`updated` AS `Updated` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexDefaultValuesGridServer.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexDefaultValuesGridServer records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexDefaultValuesGridServer' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexDefaultValuesGridServer' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    public function getDefaults($createParams=null, $clientData=array()) {
        try {
            $attributes=array();
            $attributes['id'] = 0;
            if(array_key_exists('rent_id', DIB::$USER)) 
                $attributes['client_id'] = DIB::$USER['rent_id']; // dibUser_ fields
            elseif(isset($clientData[trim('alias_parent_id')])) // See if clientData variable is referenced
                $attributes['client_id'] = $clientData[trim('alias_parent_id')];
            else // Try getting value from pef_param
                $attributes['client_id'] = EvalCriteria::evalParam('alias_parent_id', $clientData);
            $attributes['notes'] = 'Some hard coded notes';
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 254, 'test_client_contact', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Client", "First Name", "Last Name", "Notes", "Updated" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexDefaultValuesGridServer', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexDefaultValuesGridServer",
            ...
            ", ", \r\n"),
             ...
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib client_id (6167) dib_*/
				LEFT JOIN `test_client` `test_client1000` ON 
