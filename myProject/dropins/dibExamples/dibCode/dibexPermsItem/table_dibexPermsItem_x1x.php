<?php 
    Class table_dibexPermsItem_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "vip"=>PDO::PARAM_BOOL, "phone"=>PDO::PARAM_STR, "start_date"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "name"=>'none', "vip"=>'none', "phone"=>'none', "start_date"=>'none', "updated"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexPermsItem specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_client`.`id`,`test_client`.`name`,`test_client`.`start_date`,`test_client`.`phone`,`test_client`.`vip` 
                         FROM `test_client` 
                        ";
                $sql .= "WHERE $criteria";
                if(strpos($sql, '{:/') !== FALSE) { 
                ...
                // Only process fields that contain field-level permissions
                $fieldRst = array(array("field_name" => "phone", "read_criteria" => "(:vip=1)"));
                // Loop through fields that contain permissions
                foreach ($fieldRst  as $fld => $permRecord) {
                    $result = EvalCriteria::evalCrit($permRecord["read_criteria"], $attributes);
                    if ($result === FALSE)
                        $attributes[$permRecord["field_name"]] = null;
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_client`.`id` AS `Id`, `test_client`.`name` AS `Name`, `test_client`.`phone` AS `Phone`, `test_client`.`vip` AS `VIP`, `test_client`.`start_date` AS `Start Date` 
            FROM `test_client` 
                 ";
else
    $sql = "SELECT `test_client`.`id`,`test_client`.`name`,`test_client`.`start_date`,`test_client`.`phone`,`test_client`.`vip` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
	            ...
	            foreach ($attributes as $key => $record) {
	                // Loop through fields that contain permissions
	                foreach ($fieldRst  as $fld => $permRecord) {
	                    $result = EvalCriteria::evalCrit($permRecord["read_criteria"], $record);
	                    if ($result===FALSE)
	                        $attributes[$key][$permRecord["field_name"]] = null;
	                    elseif(is_array($result))
	                        return $result;
	                }
	            }
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `name` is in the design of container with id 243. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 438 are submitted to the server (ie they exist as (hidden) fields in container id 243).\r\n".
            ...
            // Field-level permission criteria is accomplished by only adding criteria for protected fields that have values (are not empty)
            // (Technically this can be accomplished by record-level criteria (field='' OR ...)
            if (!empty($attributes["phone"])) {
                $result = EvalCriteria::evalCrit("(:vip=1)", $attributes);
                if ($result===FALSE)
                    return array('error',"Add operation cancelled. The value for 'phone' does not conform to the required rules.");
                elseif(is_array($result))
                    return $result;
            }
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            // unset fields where crud_include=0 or user does not have permissions.
            if(isset($attributes["name"])) unset($attributes["name"]);
            if(isset($attributes["vip"])) unset($attributes["vip"]);
            if(isset($attributes["updated"])) unset($attributes["updated"]);
            ...
            $validAttributes = $this->removeSecuredColumns($attributes);
            if (count($validAttributes) === 0) //Nothing to update
                return array('error',"Permissions restricts you from changing any fields.");
            // Field-level permissions is accomplished by only adding criteria of protected fields that have changed
            // Note if user may not change a field, but they did (hacked it), it won't be in $validAttributes and therefore not a concern.
            $fieldCrit = $this->getFieldChangeCriteria($validAttributes, $recordOld);
            if ($fieldCrit !== '')
                $crit = $crit . ' AND (' . $fieldCrit . ')';
            // Field-level permission criteria is accomplished by only adding criteria for protected fields that have values (are not empty)
            // (Technically this can be accomplished by record-level criteria (field='' OR ...)
            if (!empty($attributes["phone"])) {
                $result = EvalCriteria::evalCrit("(:vip=1)", $attributes);
                if ($result===FALSE)
                    return array('error',"Update operation cancelled. The value for 'phone' does not conform to the required rules.");
                elseif(is_array($result))
                    return $result;
            }
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`, `start_date`, `phone`, `vip`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexPermsItem.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexPermsItem records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexPermsItem' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexPermsItem' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    private function getFieldChangeCriteria(&$validAttributes, &$oldValues) {     
         $fieldRst = array(array("field_name" => "start_date", "update_criteria_old" => "(vip=1)"),array("field_name" => "phone", "update_criteria_old" => "(vip=1)"));
        $fieldCrit = '';
        foreach ($fieldRst AS $key => $record) {
            // Criteria added for Updates only if value changed
            $fld = $record["field_name"];
            if (array_key_exists($fld, $validAttributes)) {
                if ($validAttributes[$fld] !== $oldValues[$fld]) {
                    // Build field criteria (Note, $validAttributes excludes possibilities of update_criteria_old = "")
                    if ($record["update_criteria_old"] !== "*") {
                        if (trim($fieldCrit) === '')
                            $fieldCrit = '(' . $record["update_criteria_old"] . ')';
                        else
                            $fieldCrit .= ' AND (' . $record["update_criteria_old"] . ') ';
                    }
                }
            }
        }
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 243, 'test_client', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    private function removeSecuredColumns(&$attributes) {
        ...
        $validAttributes = array("id"=>null, "phone"=>null, "start_date"=>null);
        return array_intersect_key($attributes, $validAttributes);
    }
    public function getCaptions() {
    	return array("Id", "Name", "VIP", "Phone", "Start Date" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexPermsItem', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexPermsItem",
					 'selectFields' => "`test_client`.`id`,`test_client`.`name`,`test_client`.`start_date`,`test_client`.`phone`,`test_client`.`vip`",
