<?php 
    Class table_dibexGridBasics_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_STR, "name"=>PDO::PARAM_STR, "start_date"=>PDO::PARAM_STR, "phone"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "address"=>PDO::PARAM_STR, "city_town"=>PDO::PARAM_STR, "country"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "name"=>'none', "start_date"=>'none', "phone"=>'none', "email"=>'none', "address"=>'none', "city_town"=>'none', "country"=>'none', "updated"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexGridBasics specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were updated but code was not regenerated, or the Child Container field was not linked. Please fix the configuration, regenerate code and try again.");
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_client`.`id`,`test_client`.`name`,`test_client`.`start_date`,`test_client`.`phone`,`test_client`.`email`,`test_client`.`address`,`test_client`.`city_town`,`test_client`.`country`,`test_client`.`updated` 
                         FROM `test_client` 
                        ";
                $sql .= "WHERE $criteria";
                if(strpos($sql, '{:/') !== FALSE) { 
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_client`.`id` AS `Id`, `test_client`.`name` AS `Name`, `test_client`.`start_date` AS `Start Date`, `test_client`.`phone` AS `Phone`, `test_client`.`email` AS `Email`, `test_client`.`address` AS `Address`, `test_client`.`city_town` AS `City Town`, `test_client`.`country` AS `Country`, `test_client`.`updated` AS `Updated` 
            FROM `test_client` 
                 ";
else
    $sql = "SELECT `test_client`.`id`,`test_client`.`name`,`test_client`.`start_date`,`test_client`.`phone`,`test_client`.`email`,`test_client`.`address`,`test_client`.`city_town`,`test_client`.`country`,`test_client`.`updated` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `name` is in the design of container with id 179. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 438 are submitted to the server (ie they exist as (hidden) fields in container id 179).\r\n".
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`, `start_date`, `phone`, `email`, `address`, `city_town`, `country`, `updated`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexGridBasics.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexGridBasics records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexGridBasics' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexGridBasics' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 179, 'test_client', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Name", "Start Date", "Phone", "Email", "Address", "City Town", "Country", "Updated" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexGridBasics', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexGridBasics",
					 'selectFields' => "`test_client`.`id`,`test_client`.`name`,`test_client`.`start_date`,`test_client`.`phone`,`test_client`.`email`,`test_client`.`address`,`test_client`.`city_town`,`test_client`.`country`,`test_client`.`updated`",
