<?php
class sql_dibexExecuteChildSqlWithParams_x1x {
    public $page = 1;
	public $page_size = 50;
    public $fieldType = array();
    // format: $itemName => array($expression, $contentType, $columnAlias)
    public $sqlFields = array(
            'id'=>array("tc.id", 'other', "id"),
			'name'=>array("tc.name", 'other', "name"),
			'ttname'=>array("tt.name", 'other', "ttname"),
			'tt2name'=>array("tt2.name", 'other', "tt2name"),
    );
    public $pkeys = '';
    ...
    public function read($page, $page_size=20, $order='', $gridFilter=null, $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        try {
            ...
            $userCrit = '';
            $criteria = "";
             $criteria .= " AND (tc.id < 50000 _whereclause_
{:/dropins/dibExamples/TestSqlParams/replaceCrit})";
            $this->page = $page;
            $this->page_size = $page_size;
            $this->filterParams = $filterParams;
            if($page === 1 || $countMode==='all'){
                $totalCrit = ($criteria) ? ' WHERE ' . ltrim($criteria, ' AND') : '';
                $sqlTotal =  " SELECT  Count(*) AS counter FROM test_company tc
left join testtemp tt on tc.id = tt.id
left join testtemp2 tt2 on tc.id = tt2.id
  $totalCrit";
            $sqlTotal = SqlTools::resolvePhpParam('dibexExecuteChildSqlWithParams', $sqlTotal, $params, $filterParams);
            if(is_array($sqlTotal))
                return $sqlTotal;
		...
		$sql = " SELECT  tc.id, tc.name, tt.name as ttname, tt2.name as tt2name 
        	FROM test_company tc
left join testtemp tt on tc.id = tt.id
left join testtemp2 tt2 on tc.id = tt2.id
        	$whereCrit $orderStr $limit"; 
            $sql = SqlTools::resolvePhpParam('dibexExecuteChildSqlWithParams', $sql, $params, $filterParams);
            if(is_array($sql))
                return $sql;
...
left join testtemp2 tt2 on tc.id = tt2.id
  $totalCrit";
    ...
    public function getCaptions() {
    	return array("Id", "Name", "TempTable1 name", "TempTable2 name" );
    }
    public function getSqlParts() {
		return array('model' => 'Sql',
				     'containerName' => "Sqlxx1xxdibexExecuteChildSqlWithParams",
					 'primary_field' => "",
                     'display_field' => "",                     
				     'select' => "tc.id, tc.name, tt.name as ttname, tt2.name as tt2name",
				     'from' =>   "test_company tc
left join testtemp tt on tc.id = tt.id
left join testtemp2 tt2 on tc.id = tt2.id
 ",
				     'where' =>  "tc.id < 50000 _whereclause_
{:/dropins/dibExamples/TestSqlParams/replaceCrit}", 
				     'group_by' =>"",
				     'having' =>  "",
                     'order_by' => "",
