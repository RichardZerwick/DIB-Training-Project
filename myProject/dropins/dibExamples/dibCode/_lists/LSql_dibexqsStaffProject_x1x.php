...
class LSql_dibexqsStaffProject_x1x {    
	...
	*/
    public function getList($containerItemId, $page=1, $page_size=0, $query='', $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        ...
        ;
        // Set SQL parts 
        $totalSql = " SELECT Count(*) AS totalcount FROM test_project tp 
LEFT JOIN test_staff_project tsp ON tsp.project_id = tp.id ";
        $sql = "    DISTINCT tp.id AS id,  tp.name AS id_display_value ";
        $from_clause = "test_project tp 
LEFT JOIN test_staff_project tsp ON tsp.project_id = tp.id";
        $display_field = "tp.name";
        $pkey = "tp.id";
        $order_by = ($phpFilter==='') ? " ORDER BY tp.name" : ''; 
        $criteria = "tsp.staff_id = :dibUser_staff_id";  
		// Add SQL PDO parameter values in criteria
$params[':dibUser_staff_id'] = DIB::$USER['staff_id'];
		$permParams = $params; // Remember criteria and params in case $query starts with p__ later on which will override $criteria and $params
		$permCrit = "tsp.staff_id = :dibUser_staff_id";
        // Process user filter
        if(!empty($query)) {
...
if(array_key_exists('staff_id', DIB::$USER)) {
    $params[':dibUser_staff_id']=DIB::$USER['staff_id'];
    if(array_key_exists('dibUser_staff_id', $filterParams)) unset($filterParams['dibUser_staff_id']);
}
