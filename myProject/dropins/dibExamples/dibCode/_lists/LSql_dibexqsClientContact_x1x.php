...
class LSql_dibexqsClientContact_x1x {    
	...
	*/
    public function getList($containerItemId, $page=1, $page_size=0, $query='', $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        ...
        ;
        // Set SQL parts 
        $totalSql = " SELECT Count(*) AS totalcount FROM test_client_contact cc ";
        $sql = "     cc.id AS id,  CONCAT(cc.first_name, ' ', cc.last_name) AS id_display_value ";
        $from_clause = "test_client_contact cc";
        $display_field = "CONCAT(cc.first_name, ' ', cc.last_name)";
        $pkey = "cc.id";
        $order_by = ($phpFilter==='') ? " ORDER BY cc.first_name, cc.last_name" : ''; 
        // Handle filters
        if ($activeFilter){
            $userFilter = '';
            if($containerItemId === '5498' && $activeFilter === 'dibexEventBasics_clientContactId') {
                $userFilter = "cc.client_id = :alias_self_clientId"; 
if (array_key_exists('alias_self_clientId',$filterParams)) {
    ...
    // unset($filterParams["alias_self_clientId"]);
} else {
    $value = EvalCriteria::evalParam(':alias_self_clientId', $filterParams);
    if(is_array($value))
        //return array('error',"Error! The filter parameter 'alias_self_clientId' for filter 'dibexEventBasics_clientContactId' on ^containerName^ is missing from submitted values.");
        ...
        $params[':alias_self_clientId'] = $value;
            ...
            if($containerItemId === '6845' && $activeFilter === 'dibexComponentsSelects_clientContactId') {
                $userFilter = "cc.client_id = :alias_self_clientId"; 
if (array_key_exists('alias_self_clientId',$filterParams)) {
    $params[':alias_self_clientId'] = $filterParams['alias_self_clientId'];
    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
    ...
    $value = EvalCriteria::evalParam(':alias_self_clientId', $filterParams);
    if(is_array($value))
        //return array('error',"Error! The filter parameter 'alias_self_clientId' for filter 'dibexComponentsSelects_clientContactId' on ^containerName^ is missing from submitted values.");
        $crit = '1 = 2'; // We're returning no records since if eg selected is used and there are no selected records then this error will occur.
    else 
        $params[':alias_self_clientId'] = $value;
}
            } else
            if($containerItemId === '7031' && $activeFilter === 'dibexFilterDropdown_clientContactId') {
                $userFilter = "cc.client_id = :alias_self_clientId"; 
if (array_key_exists('alias_self_clientId',$filterParams)) {
    $params[':alias_self_clientId'] = $filterParams['alias_self_clientId'];
    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
    ...
    $value = EvalCriteria::evalParam(':alias_self_clientId', $filterParams);
    if(is_array($value))
        //return array('error',"Error! The filter parameter 'alias_self_clientId' for filter 'dibexFilterDropdown_clientContactId' on ^containerName^ is missing from submitted values.");
        $crit = '1 = 2'; // We're returning no records since if eg selected is used and there are no selected records then this error will occur.
    else 
        $params[':alias_self_clientId'] = $value;
}
            ...
            if(!empty($userFilter))
            	$criteria = ($criteria !== '') ? "($criteria) AND ($userFilter)" : $userFilter;
        }
        // Process user filter
        if(!empty($query)) {
