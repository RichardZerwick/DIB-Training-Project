...
class LSql_dibexqsClientsWithProjects_x1x {    
	...
	*/
    public function getList($containerItemId, $page=1, $page_size=0, $query='', $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        ...
        ;
        // Set SQL parts 
        $totalSql = " SELECT Count(*) AS totalcount FROM test_client tc
INNER JOIN test_project tp ON tp.client_id= tc.id
INNER JOIN test_staff_project tsp ON tsp.project_id = tp.id ";
        $sql = "    DISTINCT tc.id AS id,  tc.name AS id_display_value ";
        $from_clause = "test_client tc
INNER JOIN test_project tp ON tp.client_id= tc.id
INNER JOIN test_staff_project tsp ON tsp.project_id = tp.id";
        $display_field = "tc.name";
        $pkey = "tc.id";
        $order_by = ($phpFilter==='') ? " ORDER BY tc.name" : ''; 
        // Process user filter
        if(!empty($query)) {
