...
class LSql_dibexqsStaffOnProjects_x1x {    
	...
	*/
    public function getList($containerItemId, $page=1, $page_size=0, $query='', $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        ...
        ;
        // Set SQL parts 
        $totalSql = " SELECT Count(*) AS totalcount FROM test_staff s LEFT JOIN test_staff_project sp ON sp.staff_id = s.id ";
        $sql = "    DISTINCT s.id AS id,  CONCAT(s.first_name, ' ', s.last_name) AS id_display_value ";
        $from_clause = "test_staff s LEFT JOIN test_staff_project sp ON sp.staff_id = s.id";
        $display_field = "CONCAT(s.first_name, ' ', s.last_name)";
        $pkey = "s.id";
        $order_by = ($phpFilter==='') ? " ORDER BY CONCAT(s.first_name, ' ', s.last_name)" : ''; 
        $criteria = "sp.id IS NOT NULL";  
        // Process user filter
        if(!empty($query)) {
