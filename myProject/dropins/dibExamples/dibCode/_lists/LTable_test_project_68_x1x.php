<?php
class LTable_test_project_68_x1x {
	...
	*/
    public function getList($containerItemId, $page=1, $page_size=0, $query=null, $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        try {
            ...
            $params = array();
            $sql = "`test_project`.`id` AS id, `test_project`.`name` AS `id_display_value`";
            $from_clause = "`test_project`";
            $display_field = "`test_project`.`name`";
            $pkey = "`test_project`.`id`";
            $order_by = (empty($phpFilter)) ? " ORDER BY `test_project`.`name`" : '';
            $totalSql = "SELECT count(*) AS totalcount FROM `test_project` ";
            // Add sql to show only used values (especially for filter dropdowns) - does not work for mssql...
            if($showUsedOnly) { 
            	$itemTables = array(7026=>"`test_staff`.`main_project_id`", 4212=>"`test_staff_project`.`project_id`", 7019=>"`test_staff_project`.`project_id`", 7184=>"`test_staff_project`.`project_id`");
            	if(isset($itemTables[$containerItemId])) {
            		$foreignParts = explode('.', $itemTables[$containerItemId]);
					$addSql = " INNER JOIN " . $foreignParts[0] . " dib___F ON dib___F." . $foreignParts[1] . " = `test_project`.`id` ";
            ...
            if($pageNoFromValue) {
            	...
            	$rst = dibMySqlPdo::execute("SELECT `test_project`.`name` AS dib__Display FROM `test_project` WHERE `test_project`.`id` = :pkey", $this->dbIndex, array(':pkey'=>$pageNoFromValue), TRUE, PDO::FETCH_ASSOC);
	            if($rst === FALSE)
	                return array('error', "Error! Could not read dropdown data information. Please contact the System Administrator");
				if($criteria) 
					$criteria = " WHERE $criteria AND `test_project`.`name` <= :dib__Display $order_by";
				else 
					$criteria = " WHERE `test_project`.`name` <= :dib__Display $order_by";
