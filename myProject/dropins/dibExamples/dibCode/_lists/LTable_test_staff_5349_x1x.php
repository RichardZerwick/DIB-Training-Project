<?php
class LTable_test_staff_5349_x1x {
	...
	*/
    public function getList($containerItemId, $page=1, $page_size=0, $query=null, $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        try {
            ...
            $params = array();
            $sql = "`test_staff`.`id` AS id, CONCAT(`test_staff`.`first_name`, ' ', `test_staff`.`last_name`) AS `id_display_value`";
            $from_clause = "`test_staff`";
            $display_field = "CONCAT(`test_staff`.`first_name`, ' ', `test_staff`.`last_name`)";
            $pkey = "`test_staff`.`id`";
            $order_by = (empty($phpFilter)) ? " ORDER BY CONCAT(`test_staff`.`first_name`, ' ', `test_staff`.`last_name`)" : '';
            $totalSql = "SELECT count(*) AS totalcount FROM `test_staff` ";
            // Add sql to show only used values (especially for filter dropdowns) - does not work for mssql...
            if($showUsedOnly) { 
            	$itemTables = array(275609=>"`test_project`.`project_leader_id`", 275616=>"`test_project`.`project_leader_id`", 284908=>"`test_project`.`project_leader_id`", 275661=>"`test_staff_project`.`staff_id`", 291760=>"`test_staff_project`.`staff_id`", 280605=>"`test_staff`.`manager_id`", 282038=>"`test_staff`.`manager_id`");
            	if(isset($itemTables[$containerItemId])) {
            		$foreignParts = explode('.', $itemTables[$containerItemId]);
					$addSql = " INNER JOIN " . $foreignParts[0] . " dib___F ON dib___F." . $foreignParts[1] . " = `test_staff`.`id` ";
            ...
            if($pageNoFromValue) {
            	...
            	$rst = dibMySqlPdo::execute("SELECT CONCAT(`test_staff`.`first_name`, ' ', `test_staff`.`last_name`) AS dib__Display FROM `test_staff` WHERE `test_staff`.`id` = :pkey", $this->dbIndex, array(':pkey'=>$pageNoFromValue), TRUE, PDO::FETCH_ASSOC);
	            if($rst === FALSE)
	                return array('error', "Error! Could not read dropdown data information. Please contact the System Administrator");
				if($criteria) 
					$criteria = " WHERE $criteria AND CONCAT(`test_staff`.`first_name`, ' ', `test_staff`.`last_name`) <= :dib__Display $order_by";
				else 
					$criteria = " WHERE CONCAT(`test_staff`.`first_name`, ' ', `test_staff`.`last_name`) <= :dib__Display $order_by";
