...
class LSql_dibexDynamicColumnsList_x1x {    
	...
	*/
    public function getList($containerItemId, $page=1, $page_size=0, $query='', $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        ...
        ;
        // Set SQL parts 
        $totalSql = " SELECT Count(*) AS totalcount FROM test_staff_skill tss 
...
INNER JOIN test_project tp ON tsp.project_id = tp.id
 ...
 ";
        $sql = "    DISTINCT REPLACE(CONCAT(ts.first_name, ts.last_name), ' ', '') AS id,  CONCAT(ts.first_name, ' ', ts.last_name) AS id_display_value ";
        $from_clause = "test_staff_skill tss 
INNER JOIN test_staff ts ON tss.staff_id = ts.id
INNER JOIN test_staff_project tsp ON tss.staff_id = tsp.staff_id
INNER JOIN test_project tp ON tsp.project_id = tp.id";
        $display_field = "CONCAT(ts.first_name, ' ', ts.last_name)";
        $pkey = "REPLACE(CONCAT(ts.first_name, ts.last_name), ' ', '')";
        // Process user filter
        if(!empty($query)) {
