<?php 
    Class table_dibexFormParentSqlFields_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "main_contact_id"=>PDO::PARAM_INT, "main_contact"=>PDO::PARAM_STR, "mobile"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "name"=>'none', "main_contact_id"=>'dropdown', "main_contact"=>'dibsqli', "mobile"=>'dibsqli', "email"=>'dibsqli');
    // List of items based on 'sql insert' expresions
    public $sqlFields = array('email'=>"`test_client_contact1001`.`email`", 'mobile'=>"`test_client_contact1001`.`mobile`", 'main_contact'=>"CONCAT(cc.last_name, ', ', cc.first_name, ' - ', cc.phone_w)");
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexFormParentSqlFields specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        ...
        if(!empty($activeFilter)) {
            ...
            $fromClause = "`test_client`
                /*_dib main_contact_id (4516) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id`
				/*_dib email (4517) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1001` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1001`.`id` 
                LEFT JOIN test_client_contact cc ON test_client.main_contact_id = cc.id
                ";
        } else  
            $fromClause = "`test_client` LEFT JOIN test_client_contact cc ON test_client.main_contact_id = cc.id ";
        $criteria .= $permsCrit;
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_client`.`name`,`test_client`.`main_contact_id`,`test_client`.`id` 
            , CONCAT(cc.last_name, ', ', cc.first_name, ' - ', cc.phone_w) AS `main_contact`
                               , CONCAT(`test_client_contact1000`.`first_name`, '-', `test_client_contact1000`.`last_name`) as `main_contact_id_display_value`
				, `test_client_contact1001`.`email` as `email`
				, `test_client_contact1001`.`mobile` as `mobile`
                         FROM `test_client` 
                         /*_dib main_contact_id (4516) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id`
                ...
                if($page === 1 || $countMode==='all'){
                    $sql = "SELECT Count(*) AS totalcount FROM `test_client` LEFT JOIN test_client_contact cc ON test_client.main_contact_id = cc.id  " . (!empty($criteria) ? 'WHERE ' . substr($criteria, 4) : '');
				...
				LEFT JOIN `test_client_contact` `test_client_contact1001` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1001`.`id`";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
                        $sql = "SELECT Count(*) AS filteredcount FROM `test_client` $join LEFT JOIN test_client_contact cc ON test_client.main_contact_id = cc.id  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);                          
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_client`.`name` AS `Name`, `test_client`.`id` AS `Id` 
                   , CONCAT(`test_client_contact1000`.`first_name`, '-', `test_client_contact1000`.`last_name`) as `Main Contact`
				, `test_client_contact1001`.`email` as `Email (Parent Field)`
				, `test_client_contact1001`.`mobile` as `Mobile (Parent Field)` 
            , CONCAT(cc.last_name, ', ', cc.first_name, ' - ', cc.phone_w) AS `Contact Name (Sql Insert Field)`
            FROM `test_client` 
                 /*_dib main_contact_id (4516) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id`
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `name` is in the design of container with id 181. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 438 are submitted to the server (ie they exist as (hidden) fields in container id 181).\r\n".
        ...
        try {    
            ...
            if(isset($attributes['mobile']) && trim((string)($attributes['mobile'])) !== '') {
                if(strlen($attributes["mobile"]) > 35)
                    return array ('error',"The 'Mobile (Parent Field)' field cannot contain more than 35 characters");
            }               
            // main_contact_id (integer)
            if(isset($attributes['main_contact_id']) && trim((string)($attributes['main_contact_id'])) !== '') {
                if(!is_int((int)$attributes["main_contact_id"]) || !ctype_digit((string)abs((int)$attributes["main_contact_id"])))
            ...
            if(isset($attributes['email']) && trim((string)($attributes['email'])) !== '') {
                if(!filter_var($attributes["email"], FILTER_VALIDATE_EMAIL))
                    return array ('error',"The 'Email (Parent Field)' field must be a valid email address.");
                if(strlen($attributes["email"]) > 150)
                    return array ('error',"The 'Email (Parent Field)' field cannot contain more than 150 characters");
            }               
            // id (integer)
            ...
            // Update fields in parent table test_client_contact
			if(!is_null($attributes['main_contact_id']) && trim((string)$attributes['main_contact_id'])!=='') { // Beware... the PHP empty function catches 0,'0',false,etc.       
	            $sql = "UPDATE `test_client_contact` SET 
	            		`email` = :email,`mobile` = :mobile WHERE id = :id";
		        // Set parameter values
		        $updatePar = array();
		        $updatePar[':email'] = $attributes['email'];
		        // Check if value has changed
		        if($attributes['email']!==$recordOld['email'])
		        	$parentFieldChanged = true;
		        $updatePar[':mobile'] = $attributes['mobile'];
		        // Check if value has changed
		        if($attributes['mobile']!==$recordOld['mobile'])
		        	$parentFieldChanged = true;
		        $updatePar[':id'] = $attributes['main_contact_id'];
	            $parentSuccess = dibMySqlPdo::execute($sql, $this->dbIndex, $updatePar);
	            if ($parentSuccess === FALSE) 
	            	return array('error',"Could not update the 'test_client_contact' parent table fields. Please contact the System Administrator.");
            }
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `name`, `main_contact_id`, `id`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexFormParentSqlFields.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexFormParentSqlFields records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexFormParentSqlFields' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexFormParentSqlFields' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 181, 'test_client', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Name", "Main Contact", "Contact Name (Sql Insert Field)" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexFormParentSqlFields', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexFormParentSqlFields",
					 'selectFields' => "`test_client`.`name`,`test_client`.`main_contact_id`,`test_client`.`id`",
            ...
            ", ", \r\n"),
             ...
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib main_contact_id (4516) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id`
				/*_dib email (4517) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1001` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1001`.`id` /*_dib EXTRA-TABLE-JOIN dib_*/ LEFT JOIN test_client_contact cc ON test_client.main_contact_id = cc.id", ", \r\n"),
                     'order_by' => "",
                     'filter' => ""
        );
	}
