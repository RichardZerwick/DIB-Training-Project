<?php 
    Class table_dibexTestStaffGrid_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "first_name"=>PDO::PARAM_STR, "last_name"=>PDO::PARAM_STR, "staff_code"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "mobile"=>PDO::PARAM_STR, "skype"=>PDO::PARAM_STR, "position"=>PDO::PARAM_STR, "join_date"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "first_name"=>'none', "last_name"=>'none', "staff_code"=>'none', "email"=>'none', "mobile"=>'none', "skype"=>'none', "position"=>'none', "join_date"=>'none', "notes"=>'none', "updated"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexTestStaffGrid specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_staff`.`id`,`test_staff`.`first_name`,`test_staff`.`last_name`,`test_staff`.`staff_code`,`test_staff`.`email`,`test_staff`.`mobile`,`test_staff`.`skype`,`test_staff`.`position`,`test_staff`.`join_date`,`test_staff`.`notes`,`test_staff`.`updated` 
                         FROM `test_staff` 
                        ";
                $sql .= "WHERE $criteria";
                if(strpos($sql, '{:/') !== FALSE) { 
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_staff`.`id` AS `Id`, `test_staff`.`first_name` AS `First Name`, `test_staff`.`last_name` AS `Last Name`, `test_staff`.`staff_code` AS `Staff Code`, `test_staff`.`email` AS `Email`, `test_staff`.`mobile` AS `Mobile`, `test_staff`.`skype` AS `Skype`, `test_staff`.`position` AS `Position`, `test_staff`.`join_date` AS `Join Date`, `test_staff`.`notes` AS `Notes`, `test_staff`.`updated` AS `Updated` 
            FROM `test_staff` 
                 ";
else
    $sql = "SELECT `test_staff`.`id`,`test_staff`.`first_name`,`test_staff`.`last_name`,`test_staff`.`staff_code`,`test_staff`.`email`,`test_staff`.`mobile`,`test_staff`.`skype`,`test_staff`.`position`,`test_staff`.`join_date`,`test_staff`.`notes`,`test_staff`.`updated` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
	        ...
	        // skype (plain text)
            if(isset($attributes['skype']) && trim((string)($attributes['skype'])) !== '') {
                if(strlen($attributes["skype"]) > 40)
                    return array ('error',"The 'Skype' field cannot contain more than 40 characters");
            }               
	        // join_date (date)
            ...
            if(!array_key_exists('email', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `email` is in the design of container with id 167. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 462 are submitted to the server (ie they exist as (hidden) fields in container id 167).\r\n".
            ...
            if(!array_key_exists('first_name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `first_name` is in the design of container with id 167. It is required since it is involved in checking the unique index\r\n".
            ...
            if(!array_key_exists('last_name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `last_name` is in the design of container with id 167. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 464 are submitted to the server (ie they exist as (hidden) fields in container id 167).\r\n".
                         "NOTE: if one of these fields is a dropdown or other component that has a port that points to a container which the current permission group does NOT have access to, this component will not be included. Simply remove the link to the Port record and regenerate CRUD file.");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
            ...
            if(!array_key_exists('staff_code', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `staff_code` is in the design of container with id 167. It is required since it is involved in checking the unique index\r\n".
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 468 are submitted to the server (ie they exist as (hidden) fields in container id 167).\r\n".
                         "NOTE: if one of these fields is a dropdown or other component that has a port that points to a container which the current permission group does NOT have access to, this component will not be included. Simply remove the link to the Port record and regenerate CRUD file.");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `first_name`, `last_name`, `staff_code`, `email`, `mobile`, `skype`, `position`, `join_date`, `notes`, `updated`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexTestStaffGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexTestStaffGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexTestStaffGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexTestStaffGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 167, 'test_staff', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "First Name", "Last Name", "Staff Code", "Email", "Mobile", "Skype", "Position", "Join Date", "Notes", "Updated" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexTestStaffGrid', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexTestStaffGrid",
					 'selectFields' => "`test_staff`.`id`,`test_staff`.`first_name`,`test_staff`.`last_name`,`test_staff`.`staff_code`,`test_staff`.`email`,`test_staff`.`mobile`,`test_staff`.`skype`,`test_staff`.`position`,`test_staff`.`join_date`,`test_staff`.`notes`,`test_staff`.`updated`",
