<?php 
    Class table_dibexComponentsFiles_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "skill_definition_file"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "name"=>'none', "skill_definition_file"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexComponentsFiles specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        ...
        if($getFirstOnly) { // Used after deletes to navigate to first record
            $sql = "SELECT `test_skill`.`id` FROM $fromClause $criteria ORDER BY `test_skill`.`id` limit 1";
        ...
        $sql = "SELECT count(*) as dib__total, min(`test_skill`.`id`) as dib__minid, max(`test_skill`.`id`) as dib__maxid FROM $fromClause $criteria";
        $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
        if(empty($rst))
        ...
        if($totalCount>0) {
            ...
            $tempCrit = ($criteria === '') ? "WHERE `test_skill`.`id` < :pk1 $permsCrit" : "$criteria AND `test_skill`.`id` < :pk1";
            $sql = "SELECT max(`test_skill`.`id`) as dib__id, count(`test_skill`.`id`) as dib__counter FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
            ...
            // Get next
            $tempCrit = ($criteria === '') ? "WHERE `test_skill`.`id` > :pk1 $permsCrit" : "$criteria AND `test_skill`.`id` > :pk1";
            $sql = "SELECT min(`test_skill`.`id`) as dib__id FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
...
$sql = "SELECT `test_skill`.`id` 
        FROM $fromClause
        $criteria
        ORDER BY `test_skill`.`id` 
        LIMIT " . ($position - 1) . ', 1';
        $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
        if(empty($rst))
            return null;
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                // Get pkey values for removing cache related to uploaded files
                $pkid = implode('___', $params);
                $criteria = "`test_skill`.`id` = :pk1 ";
                $sql = "SELECT `test_skill`.`id`,`test_skill`.`name`,`test_skill`.`skill_definition_file` 
                         FROM `test_skill` 
                        ";
                $sql .= "WHERE $criteria";
                if(strpos($sql, '{:/') !== FALSE) { 
                ...
                // If files were uploaded to one or more items, move them to their folders
                session_start();
                $sess = DIB::$USER['unique_id'];
                if(isset($_SESSION['FileUploads']['dibexComponentsFiles'][$pkid]['skill_definition_file'])) {				
                    $file_name = $sess."_262_1008_$pkid.tmp";
                    $file_path_expr = DIB::$USERSPATH.'tmp'.DIRECTORY_SEPARATOR.$file_name;
                    if(isset($_SESSION['FileUploads']['dibexComponentsFiles'][$pkid]['skill_definition_file']))
                        unset($_SESSION['FileUploads']['dibexComponentsFiles'][$pkid]['skill_definition_file']);
                    if(is_file($file_path_expr)) 
                        unlink($file_path_expr);
                }
                session_write_close();
                $filteredCount = 1;
                $totalCount = 1; 
                ...
                if($page === 1 || $countMode==='all'){
                    $sql = "SELECT Count(*) AS totalcount FROM `test_skill`   " . (!empty($criteria) ? 'WHERE ' . substr($criteria, 4) : '');
                ...
                if ($criteria !== '') {
                    ...
                    if($usedSqlField || !empty($activeFilter)) {
						$join = "
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
                        $sql = "SELECT Count(*) AS filteredcount FROM `test_skill` $join   $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`test_skill`.`' . $record['property'] . '` ' . $direction . ', ';
                    }
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_skill`.`id` AS `Id`, `test_skill`.`name` AS `Name`, `test_skill`.`skill_definition_file` AS `fileUpload with Image` 
            FROM `test_skill` 
                 ";
else
    $sql = "SELECT `test_skill`.`id`,`test_skill`.`name`,`test_skill`.`skill_definition_file` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(isset($attributes['name']) && trim((string)($attributes['name'])) !== '') {
                if(strlen($attributes["name"]) > 50)
                    return array ('error',"The 'Name' field cannot contain more than 50 characters");
            }  else
                return array ('error',"The 'Name' field is required. Please provide a value.");
	        // skill_definition_file ()
            if(isset($attributes['skill_definition_file']) && trim((string)($attributes['skill_definition_file'])) !== '') {
            }               
            //Check Unique Values for table option 459
            $criteria = '1=1';
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `name` is in the design of container with id 262. It is required since it is involved in checking the unique index\r\n".
                         "of pef_table_option_field.id 479. Note, you may need to add it as a hidden field in the container, or set a default in pef_field.\r\n".
            ...
            $sql = "SELECT `test_skill`.`id` AS pkv FROM `test_skill` WHERE $criteria";
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 459 are submitted to the server (ie they exist as (hidden) fields in container id 262).\r\n".
            ...
            if(!empty($rst)) {
                if($makeUniqueValues)
                    ...
                    $attributes['name'] = SyncFunctions::cleanName($attributes['name'],'test_skill', '');
            ...
            session_start();
			$sess = DIB::$USER['unique_id'];
			if(isset($_SESSION['FileUploads']['dibexComponentsFiles']['dib___NEWRECORD']['skill_definition_file'])) {
				$newName = $_SESSION['FileUploads']['dibexComponentsFiles']['dib___NEWRECORD']['skill_definition_file'];
			    $file_name = $sess.'_262_1008.tmp';
			    $file_path_expr = DIB::$USERSPATH.'tmp'.DIRECTORY_SEPARATOR.$file_name;
			    $newPath = FileUpload::getFileLocation(6854, $pkValues, $newName, 'dibexComponentsFiles');
			    unset($_SESSION['FileUploads']['dibexComponentsFiles']['dib___NEWRECORD']['skill_definition_file']);
			    if(is_file($file_path_expr)) rename($file_path_expr, $newPath[0]);
			}
            session_write_close();
            ///
            $crit = TRUE;
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue) {
                    ...
                    $this->auditInsert("create", $fieldName, null, $newValue, 69, $recordId, $newDisplayValue, $entityId);
                }
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            $params[":pk1"] = $pkValues["id"];
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            $pkCrit = "`test_skill`.`id` = :pk1";
            // Check Unique Values for table option PRIMARY(458)
            $criteria = '1=1 ';           
            if(!array_key_exists('id', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `id` FROM `test_skill` WHERE $pkCrit", $this->dbIndex, $params, true);
            ...
            // Check Unique Values for table option name(459)
            $criteria = '1=1 ';           
            if(!array_key_exists('name', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `name` FROM `test_skill` WHERE $pkCrit", $this->dbIndex, $params, true);
            ...
            // If files were uploaded to one or more items, move them to their folders
            // Get pkey values
            $pkid = implode('___', $pkValues);
            session_start();
            $sess = DIB::$USER['unique_id'];            
			if(isset($_SESSION['FileUploads']['dibexComponentsFiles'][$pkid]['skill_definition_file'])) {
				$newName = $_SESSION['FileUploads']['dibexComponentsFiles'][$pkid]['skill_definition_file'];
			    $file_name = $sess."_262_1008_$pkid.tmp";
			    $file_path_expr = DIB::$USERSPATH.'tmp'.DIRECTORY_SEPARATOR.$file_name;
			    $newPath = FileUpload::getFileLocation(6854, $pkValues, $newName, 'dibexComponentsFiles');
			    unset($_SESSION['FileUploads']['dibexComponentsFiles'][$pkid]['skill_definition_file']);
                    ...
                    if ($crit===TRUE) {
                        ...
                        foreach ($validAttributes AS $fieldName => $newValue) {
                            if ($newValue !== $recordOld[$fieldName]) {
                                ...
                                $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 69, $recordId, $newDisplayValue, $entityId, $oldDisplayValue);
                            }
                        }
                    } elseif (is_array($crit)) return $crit;
                }
    ...
    public function delete($pkValues, $clientData=array()) {
        try {
            ...
            //CHECK IF ANY FILES ARE LINKED TO THIS RECORD AND DELETE THEM ON THE SERVER IF THEY EXIST
            $result = FileUpload::deleteFile('dibexComponentsFiles',6317, $pkValues);
            if ($result === FALSE) 
            	Log::err("Item 6317 for container 'dibexComponentsFiles' is configured in the Crud class to store files." .
            			 "\r\nAttempt to resolve configuration and delete physical files to this item for primary key values '" . json_encode($pkValues) . "' failed.\r\n" .
            			 "Establish if the Crud file needs to be regenerated (perhaps the item no longer stores files, or if the webserver user has rights to delete these files.");
            $result = FileUpload::deleteFile('dibexComponentsFiles',6318, $pkValues);
            if ($result === FALSE) 
            	Log::err("Item 6318 for container 'dibexComponentsFiles' is configured in the Crud class to store files." .
            			 "\r\nAttempt to resolve configuration and delete physical files to this item for primary key values '" . json_encode($pkValues) . "' failed.\r\n" .
            			 "Establish if the Crud file needs to be regenerated (perhaps the item no longer stores files, or if the webserver user has rights to delete these files.");
            $result = FileUpload::deleteFile('dibexComponentsFiles',6854, $pkValues);
            if ($result === FALSE) 
            	Log::err("Item 6854 for container 'dibexComponentsFiles' is configured in the Crud class to store files." .
            			 "\r\nAttempt to resolve configuration and delete physical files to this item for primary key values '" . json_encode($pkValues) . "' failed.\r\n" .
            			 "Establish if the Crud file needs to be regenerated (perhaps the item no longer stores files, or if the webserver user has rights to delete these files.");
            $sql = "DELETE `test_skill` FROM `test_skill`   WHERE $crit";
            dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) {
                        ...
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 69, $recordId, null, $entityId, $oldDisplayValue);
                    }
                } elseif (is_array($crit)) return $crit;
                return true;
            }
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`, `skill_definition_file`
                    FROM `test_skill` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexComponentsFiles.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexComponentsFiles records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexComponentsFiles' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexComponentsFiles' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = "UPDATE test_skill SET `" . $args[0] . "` = :value WHERE $pkCrit";					
					DibApp::$array['DuplicateRecords']['test_skill*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 262, 'test_skill', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Name", "fileUpload with Image" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexComponentsFiles', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexComponentsFiles",
					 'selectFields' => "`test_skill`.`id`,`test_skill`.`name`,`test_skill`.`skill_definition_file`",
				     'selectSqlFields' => trim("
            ", ", \r\n"),
				     'selectSqlDisplay' =>  trim("
            ", ", \r\n"),
				     'selectTableDisplay' => trim("", ", \r\n"),          
                     'from' => trim("`test_skill` /*_dib DROPDOWNS - SQL DISPLAY EXPR dib_*/ 
