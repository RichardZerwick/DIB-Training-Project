<?php 
    Class table_dibexTestProjectGrid_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "client_id"=>PDO::PARAM_INT, "project_leader_id"=>PDO::PARAM_INT, "notes"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "name"=>'none', "client_id"=>'dropdown', "project_leader_id"=>'dropdown', "notes"=>'none', "updated"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'dibexContainerFilterPort_tabProjects') === 0) {
            ...
            if (array_key_exists('alias_parent_id',$filterParams)) {
                ...
                if(is_array($value)  || $value === ':alias_parent_id')
                    //return array('error',"Error! The filter parameter 'alias_parent_id' for filter '' on dibexTestProjectGrid is missing from submitted values.");
        ...
        if($criteria==='') {
            Log::err("The read request for container dibexTestProjectGrid specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`test_project`.`id` = :pk1 ";
                $sql = "SELECT `test_project`.`id`,`test_project`.`name`,`test_project`.`client_id`,`test_project`.`project_leader_id`,`test_project`.`notes`,`test_project`.`updated` 
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_project`.`id` AS `Id`, `test_project`.`name` AS `Name`, `test_project`.`notes` AS `Notes`, `test_project`.`updated` AS `Updated` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(isset($attributes['project_leader_id']) && trim((string)($attributes['project_leader_id'])) !== '') {
                if(!is_int((int)$attributes["project_leader_id"]) || !ctype_digit((string)abs((int)$attributes["project_leader_id"])))
                    return array ('error',"The 'Project Leader' field must be an integer value.");
                if($attributes["project_leader_id"] < -2147483647)
                    return array ('error',"The 'Project Leader' field value must be greater than or equal to -2147483647");
                if($attributes["project_leader_id"] > 2147483647)
                    return array ('error',"The 'Project Leader' field value must be less than or equal to 2147483647");
            }               
	        // notes (plain text)
            ...
            //Check Unique Values for table option 451
            $criteria = '1=1';
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `name` is in the design of container with id 166. It is required since it is involved in checking the unique index\r\n".
                         "of pef_table_option_field.id 471. Note, you may need to add it as a hidden field in the container, or set a default in pef_field.\r\n".
            ...
            $sql = "SELECT `test_project`.`id` AS pkv FROM `test_project` WHERE $criteria";
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 451 are submitted to the server (ie they exist as (hidden) fields in container id 166).\r\n".
            ...
            if(!empty($rst)) {
                if($makeUniqueValues)
                    ...
                    $attributes['name'] = SyncFunctions::cleanName($attributes['name'],'test_project', '');
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue) {
                    ...
                    $this->auditInsert("create", $fieldName, null, $newValue, 68, $recordId, $newDisplayValue, $entityId);
                }
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            // Check Unique Values for table option PRIMARY(448)
            $criteria = '1=1 ';           
            if(!array_key_exists('id', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `id` FROM `test_project` WHERE $pkCrit", $this->dbIndex, $params, true);
            ...
            // Check Unique Values for table option name(451)
            $criteria = '1=1 ';           
            if(!array_key_exists('name', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `name` FROM `test_project` WHERE $pkCrit", $this->dbIndex, $params, true);
                    ...
                    if ($crit===TRUE) {
                        ...
                        foreach ($validAttributes AS $fieldName => $newValue) {
                            if ($newValue !== $recordOld[$fieldName]) {
                                ...
                                $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 68, $recordId, $newDisplayValue, $entityId, $oldDisplayValue);
                            }
                        }
                    } elseif (is_array($crit)) return $crit;
                }
    ...
    public function delete($pkValues, $clientData=array()) {
        try {
            ...
            $sql = "DELETE `test_project` FROM `test_project`   WHERE $crit";
            dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`, `client_id`, `project_leader_id`, `notes`, `updated`
                    FROM `test_project` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexTestProjectGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexTestProjectGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexTestProjectGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexTestProjectGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = "UPDATE test_project SET `" . $args[0] . "` = :value WHERE $pkCrit";					
					DibApp::$array['DuplicateRecords']['test_project*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 166, 'test_project', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Name", "Client", "Project Leader", "Notes", "Updated" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexTestProjectGrid', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexTestProjectGrid",
					 'selectFields' => "`test_project`.`id`,`test_project`.`name`,`test_project`.`client_id`,`test_project`.`project_leader_id`,`test_project`.`notes`,`test_project`.`updated`",
            ...
            ", ", \r\n"),
             ...
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib client_id (4186) dib_*/
				LEFT JOIN `test_client` `test_client1000` ON 
                    `test_project`.`client_id` = `test_client1000`.`id`
				/*_dib project_leader_id (4187) dib_*/
				LEFT JOIN `test_staff` `test_staff1001` ON 
