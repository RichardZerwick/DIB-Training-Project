<?php 
    Class table_dibexFlexInPractice_x1x {
    public $count = 0;
    ...
    public $fieldType = array("main_contact_id"=>PDO::PARAM_INT, "address"=>PDO::PARAM_STR, "phone"=>PDO::PARAM_STR, "start_date"=>PDO::PARAM_STR, "id"=>PDO::PARAM_INT, "city_town"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR, "country"=>PDO::PARAM_STR, "zip_code"=>PDO::PARAM_STR, "name"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("main_contact_id"=>'dropdown', "address"=>'none', "phone"=>'none', "start_date"=>'none', "id"=>'none', "city_town"=>'none', "updated"=>'none', "country"=>'none', "zip_code"=>'none', "name"=>'none', "notes"=>'none', "email"=>'none');
    // List of items based on 'sql insert' expresions
    public $sqlFields = array();
    public $fkeyDisplay = array('main_contact_id'=>"CONCAT(`test_client_contact1000`.`first_name`, '-', `test_client_contact1000`.`last_name`)",
            );
    ...
    private function parseFilterArray($filterParams=array(), &$params=array(), &$fieldType=array()) {
        ...
        foreach($filterParams as $record) {
            ...
            if(array_key_exists($field, $this->sqlFields))
            	...
            	$str .= '`test_client`.`' . $field . '` = :pk' . $i . ' AND ';
            $params[':pk' . $i] = $value;
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexFlexInPractice specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        ...
        if(!empty($activeFilter)) {
            ...
            $fromClause = "`test_client`
                /*_dib main_contact_id (4978) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id` 
                ";
        } else  
            $fromClause = "`test_client`  ";
        $criteria .= $permsCrit;
        ...
        if($getFirstOnly) { // Used after deletes to navigate to first record
            $sql = "SELECT `test_client`.`id` FROM $fromClause $criteria ORDER BY `test_client`.`id` limit 1";
        ...
        $sql = "SELECT count(*) as dib__total, min(`test_client`.`id`) as dib__minid, max(`test_client`.`id`) as dib__maxid FROM $fromClause $criteria";
        $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
        if(empty($rst))
        ...
        if($totalCount>0) {
            ...
            $tempCrit = ($criteria === '') ? "WHERE `test_client`.`id` < :pk1 $permsCrit" : "$criteria AND `test_client`.`id` < :pk1";
            $sql = "SELECT max(`test_client`.`id`) as dib__id, count(`test_client`.`id`) as dib__counter FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
            ...
            // Get next
            $tempCrit = ($criteria === '') ? "WHERE `test_client`.`id` > :pk1 $permsCrit" : "$criteria AND `test_client`.`id` > :pk1";
            $sql = "SELECT min(`test_client`.`id`) as dib__id FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`test_client`.`id` = :pk1 ";
                $sql = "SELECT `test_client`.`id`,`test_client`.`name`,`test_client`.`start_date`,`test_client`.`phone`,`test_client`.`email`,`test_client`.`address`,`test_client`.`city_town`,`test_client`.`country`,`test_client`.`zip_code`,`test_client`.`notes`,`test_client`.`updated`,`test_client`.`main_contact_id` 
                               , CONCAT(`test_client_contact1000`.`first_name`, '-', `test_client_contact1000`.`last_name`) as `main_contact_id_display_value`
                         FROM `test_client` 
                         /*_dib main_contact_id (4978) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id` 
                ...
                if($page === 1 || $countMode==='all'){
                    $sql = "SELECT Count(*) AS totalcount FROM `test_client`   " . (!empty($criteria) ? 'WHERE ' . substr($criteria, 4) : '');
				...
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id`";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
                        $sql = "SELECT Count(*) AS filteredcount FROM `test_client` $join   $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`test_client`.`' . $record['property'] . '` ' . $direction . ', ';
                    }
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_client`.`id` AS `Id`, `test_client`.`name` AS `Name`, `test_client`.`start_date` AS `Start Date`, `test_client`.`phone` AS `Phone`, `test_client`.`email` AS `Email`, `test_client`.`address` AS `Address`, `test_client`.`city_town` AS `City Town`, `test_client`.`country` AS `Country`, `test_client`.`zip_code` AS `Zip Code`, `test_client`.`notes` AS `Notes`, `test_client`.`updated` AS `Updated` 
                   , CONCAT(`test_client_contact1000`.`first_name`, '-', `test_client_contact1000`.`last_name`) as `Main Contact` 
            FROM `test_client` 
                 /*_dib main_contact_id (4978) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            if(isset($attributes['name']) && trim((string)($attributes['name'])) !== '') {
                if(strlen($attributes["name"]) > 80)
                    return array ('error',"The 'Name' field cannot contain more than 80 characters");
            }  else
                return array ('error',"The 'Name' field is required. Please provide a value.");
	        // phone (plain text)
            if(isset($attributes['phone']) && trim((string)($attributes['phone'])) !== '') {
                if(strlen($attributes["phone"]) > 100)
                    return array ('error',"The 'Phone' field cannot contain more than 100 characters");
            }               
	        // email (email)
            if(isset($attributes['email']) && trim((string)($attributes['email'])) !== '') {
                ...
                if(strlen($attributes["email"]) > 100)
                    return array ('error',"The 'Email' field cannot contain more than 100 characters");
            }               
	        // address (plain text)
            if(isset($attributes['address']) && trim((string)($attributes['address'])) !== '') {
                if(strlen($attributes["address"]) > 200)
                    return array ('error',"The 'Address' field cannot contain more than 200 characters");
            }               
	        // city_town (plain text)
            if(isset($attributes['city_town']) && trim((string)($attributes['city_town'])) !== '') {
                if(strlen($attributes["city_town"]) > 30)
                    return array ('error',"The 'City Town' field cannot contain more than 30 characters");
            }               
	        // country (plain text)
            if(isset($attributes['country']) && trim((string)($attributes['country'])) !== '') {
                if(strlen($attributes["country"]) > 30)
                    return array ('error',"The 'Country' field cannot contain more than 30 characters");
            }               
	        // zip_code (plain text)
            if(isset($attributes['zip_code']) && trim((string)($attributes['zip_code'])) !== '') {
                if(strlen($attributes["zip_code"]) > 30)
                    return array ('error',"The 'Zip Code' field cannot contain more than 30 characters");
            }               
	        // start_date (date)
            if(isset($attributes['start_date']) && trim((string)($attributes['start_date'])) !== '') {
                if(!strtotime(str_replace('/', '-', $attributes["start_date"])))
                    return array ('error',"The 'Start Date' field must be a valid date.");
                else
                    $attributes["start_date"] = date('Y-m-d', strtotime(str_replace('/', '-', $attributes["start_date"])));
            }               
	        // main_contact_id (integer)
            if(isset($attributes['main_contact_id']) && trim((string)($attributes['main_contact_id'])) !== '') {
                if(!is_int((int)$attributes["main_contact_id"]) || !ctype_digit((string)abs((int)$attributes["main_contact_id"])))
                    return array ('error',"The 'Main Contact' field must be an integer value.");
                if($attributes["main_contact_id"] < -2147483647)
                    return array ('error',"The 'Main Contact' field value must be greater than or equal to -2147483647");
                if($attributes["main_contact_id"] > 2147483647)
                    return array ('error',"The 'Main Contact' field value must be less than or equal to 2147483647");
            }               
	        // notes (plain text)
            if(isset($attributes['notes']) && trim((string)($attributes['notes'])) !== '') {
                if(strlen($attributes["notes"]) > 65535)
                    return array ('error',"The 'Notes' field cannot contain more than 65535 characters");
            }               
	        // updated (datetime)
            if(isset($attributes['updated']) && trim((string)($attributes['updated'])) !== '') {
                if(!strtotime(str_replace('/', '-', $attributes["updated"])))
                    return array ('error',"The 'Updated' field must be a valid date-time.");
                else
                    $attributes["updated"] = date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $attributes["updated"])));
            }               
            //Check Unique Values for table option 438
            $criteria = '1=1';
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that field `name` is in the design of container with id 198. It is required since it is involved in checking the unique index\r\n".
                         "of pef_table_option_field.id 458. Note, you may need to add it as a hidden field in the container, or set a default in pef_field.\r\n".
            ...
            $sql = "SELECT `test_client`.`id` AS pkv FROM `test_client` WHERE $criteria";
            ...
            if ($rst === FALSE) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index\r\n".
                         "of pef_table_option.id 438 are submitted to the server (ie they exist as (hidden) fields in container id 198).\r\n".
            ...
            if(!empty($rst)) {
                if($makeUniqueValues)
                    ...
                    $attributes['name'] = SyncFunctions::cleanName($attributes['name'],'test_client', '');
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue) {
                    ...
                    $this->auditInsert("create", $fieldName, null, $newValue, 66, $recordId, $newDisplayValue, $entityId);
                }
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            $params[":pk1"] = $pkValues["id"];
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            $pkCrit = "`test_client`.`id` = :pk1";
            // Check Unique Values for table option PRIMARY(434)
            $criteria = '1=1 ';           
            if(!array_key_exists('id', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `id` FROM `test_client` WHERE $pkCrit", $this->dbIndex, $params, true);
            ...
            // Check Unique Values for table option name(438)
            $criteria = '1=1 ';           
            if(!array_key_exists('name', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `name` FROM `test_client` WHERE $pkCrit", $this->dbIndex, $params, true);
                    ...
                    if ($crit===TRUE) {
                        ...
                        foreach ($validAttributes AS $fieldName => $newValue) {
                            if ($newValue !== $recordOld[$fieldName]) {
                                ...
                                $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 66, $recordId, $newDisplayValue, $entityId, $oldDisplayValue);
                            }
                        }
                    } elseif (is_array($crit)) return $crit;
                }
    ...
    public function delete($pkValues, $clientData=array()) {
        try {
            ...
            $sql = "DELETE `test_client` FROM `test_client`   WHERE $crit";
            dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) {
                        ...
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 66, $recordId, null, $entityId, $oldDisplayValue);
                    }
                } elseif (is_array($crit)) return $crit;
                return true;
            }
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`, `start_date`, `phone`, `email`, `address`, `city_town`, `country`, `zip_code`, `notes`, `updated`, `main_contact_id`
                    FROM `test_client` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexFlexInPractice.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexFlexInPractice records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexFlexInPractice' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexFlexInPractice' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = "UPDATE test_client SET `" . $args[0] . "` = :value WHERE $pkCrit";					
					DibApp::$array['DuplicateRecords']['test_client*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 198, 'test_client', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Main Contact", "Address", "Phone", "Start Date", "Id", "City Town", "Updated", "Country", "Zip Code", "Name", "Notes", "Email" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexFlexInPractice', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexFlexInPractice",
					 'selectFields' => "`test_client`.`id`,`test_client`.`name`,`test_client`.`start_date`,`test_client`.`phone`,`test_client`.`email`,`test_client`.`address`,`test_client`.`city_town`,`test_client`.`country`,`test_client`.`zip_code`,`test_client`.`notes`,`test_client`.`updated`,`test_client`.`main_contact_id`",
				     'selectSqlFields' => trim("
            ", ", \r\n"),
				     'selectSqlDisplay' =>  trim("
            ", ", \r\n"),
				     'selectTableDisplay' => trim(", CONCAT(`test_client_contact1000`.`first_name`, '-', `test_client_contact1000`.`last_name`) as `main_contact_id_display_value`", ", \r\n"),          
                     'from' => trim("`test_client` /*_dib DROPDOWNS - SQL DISPLAY EXPR dib_*/ 
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib main_contact_id (4978) dib_*/
				LEFT JOIN `test_client_contact` `test_client_contact1000` ON 
                    `test_client`.`main_contact_id` = `test_client_contact1000`.`id` /*_dib EXTRA-TABLE-JOIN dib_*/ ", ", \r\n"),
                     'order_by' => "",
                     'filter' => ""
        );
	}
