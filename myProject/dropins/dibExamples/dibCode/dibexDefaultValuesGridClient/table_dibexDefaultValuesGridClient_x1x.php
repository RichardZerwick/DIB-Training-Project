<?php 
    Class table_dibexDefaultValuesGridClient_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "client_id"=>PDO::PARAM_INT, "first_name"=>PDO::PARAM_STR, "last_name"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "client_id"=>'dropdown', "first_name"=>'none', "last_name"=>'none', "email"=>'none', "notes"=>'none', "updated"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'dibexDefaultValuesClient_dibexDefaultValuesGridClient') === 0) {
            ...
            if (array_key_exists('alias_parent_id',$filterParams)) {
                ...
                if(is_array($value)  || $value === ':alias_parent_id')
                    //return array('error',"Error! The filter parameter 'alias_parent_id' for filter '' on dibexDefaultValuesGridClient is missing from submitted values.");
        ...
        if($criteria==='') {
            Log::err("The read request for container dibexDefaultValuesGridClient specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_client_contact`.`id`,`test_client_contact`.`client_id`,`test_client_contact`.`first_name`,`test_client_contact`.`last_name`,`test_client_contact`.`email`,`test_client_contact`.`notes`,`test_client_contact`.`updated` 
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_client_contact`.`id` AS `Id`, `test_client_contact`.`first_name` AS `First Name`, `test_client_contact`.`last_name` AS `Last Name`, `test_client_contact`.`email` AS `Email`, `test_client_contact`.`notes` AS `Notes`, `test_client_contact`.`updated` AS `Updated` 
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `client_id`, `first_name`, `last_name`, `email`, `notes`, `updated`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexDefaultValuesGridClient.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexDefaultValuesGridClient records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexDefaultValuesGridClient' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexDefaultValuesGridClient' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 255, 'test_client_contact', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Client", "First Name", "Last Name", "Email", "Notes", "Updated" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexDefaultValuesGridClient', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexDefaultValuesGridClient",
					 'selectFields' => "`test_client_contact`.`id`,`test_client_contact`.`client_id`,`test_client_contact`.`first_name`,`test_client_contact`.`last_name`,`test_client_contact`.`email`,`test_client_contact`.`notes`,`test_client_contact`.`updated`",
            ...
            ", ", \r\n"),
             ...
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib client_id (6187) dib_*/
				LEFT JOIN `test_client` `test_client1000` ON 
