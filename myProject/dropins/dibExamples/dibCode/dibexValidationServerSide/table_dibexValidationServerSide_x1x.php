<?php 
    Class table_dibexValidationServerSide_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "staff_id"=>PDO::PARAM_INT, "notes"=>PDO::PARAM_STR, "perc_complete"=>PDO::PARAM_INT, "date_from"=>PDO::PARAM_STR, "Staff_MobileNo"=>PDO::PARAM_STR, "project_id"=>PDO::PARAM_INT);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "staff_id"=>'dropdown', "notes"=>'none', "perc_complete"=>'none', "date_from"=>'none', "Staff_MobileNo"=>'dibsqli', "project_id"=>'none');
    // List of items based on 'sql insert' expresions
    public $sqlFields = array('Staff_MobileNo'=>"`test_staff1001`.`mobile`");
    public $fkeyDisplay = array('staff_id'=>"CONCAT(`test_staff1000`.`first_name`, ' ', `test_staff1000`.`last_name`)",
            );
    ...
    private function parseFilterArray($filterParams=array(), &$params=array(), &$fieldType=array()) {
        ...
        foreach($filterParams as $record) {
            ...
            if(array_key_exists($field, $this->sqlFields))
            	...
            	$str .= '`test_staff_project`.`' . $field . '` = :pk' . $i . ' AND ';
            $params[':pk' . $i] = $value;
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexValidationServerSide specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        ...
        if(!empty($activeFilter)) {
            ...
            $fromClause = "`test_staff_project`
                /*_dib staff_id1 (7192) dib_*/
				LEFT JOIN `test_staff` `test_staff1000` ON 
                    `test_staff_project`.`staff_id` = `test_staff1000`.`id`
				/*_dib Staff_MobileNo (7190) dib_*/
				LEFT JOIN `test_staff` `test_staff1001` ON 
                    `test_staff_project`.`staff_id` = `test_staff1001`.`id` 
                ";
        } else  
            $fromClause = "`test_staff_project`  ";
        $criteria .= $permsCrit;
        ...
        if($getFirstOnly) { // Used after deletes to navigate to first record
            $sql = "SELECT `test_staff_project`.`id` FROM $fromClause $criteria ORDER BY `test_staff_project`.`id` limit 1";
        ...
        $sql = "SELECT count(*) as dib__total, min(`test_staff_project`.`id`) as dib__minid, max(`test_staff_project`.`id`) as dib__maxid FROM $fromClause $criteria";
        $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
        if(empty($rst))
        ...
        if($totalCount>0) {
            ...
            $tempCrit = ($criteria === '') ? "WHERE `test_staff_project`.`id` < :pk1 $permsCrit" : "$criteria AND `test_staff_project`.`id` < :pk1";
            $sql = "SELECT max(`test_staff_project`.`id`) as dib__id, count(`test_staff_project`.`id`) as dib__counter FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
            ...
            // Get next
            $tempCrit = ($criteria === '') ? "WHERE `test_staff_project`.`id` > :pk1 $permsCrit" : "$criteria AND `test_staff_project`.`id` > :pk1";
            $sql = "SELECT min(`test_staff_project`.`id`) as dib__id FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
            if(empty($rst))
...
$sql = "SELECT `test_staff_project`.`id` 
        FROM $fromClause
        $criteria
        ORDER BY `test_staff_project`.`id` 
        LIMIT " . ($position - 1) . ', 1';
        $rst = dibMySqlPdo::execute($sql, $this->dbIndex, $params, true);
        if(empty($rst))
            return null;
    ...
    public function parseGridFilter($gridFilter, &$params=array(), &$fieldType=array(), &$usedSqlField=FALSE, &$i = 0) {
        ...
        foreach($gridFilter as $row) {
            ...
            if(array_key_exists($field, $this->sqlFields)) {
            	...
            	$fieldExpr = "`test_staff_project`.`$field`";
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`test_staff_project`.`id` = :pk1 ";
                $sql = "SELECT `test_staff_project`.`id`,`test_staff_project`.`staff_id`,`test_staff_project`.`project_id`,`test_staff_project`.`date_from`,`test_staff_project`.`notes`,`test_staff_project`.`perc_complete` 
                               , CONCAT(`test_staff1000`.`first_name`, ' ', `test_staff1000`.`last_name`) as `staff_id_display_value`
				, `test_staff1001`.`mobile` as `Staff_MobileNo`
                         FROM `test_staff_project` 
                         /*_dib staff_id1 (7192) dib_*/
				LEFT JOIN `test_staff` `test_staff1000` ON 
                    `test_staff_project`.`staff_id` = `test_staff1000`.`id`
				/*_dib Staff_MobileNo (7190) dib_*/
                ...
                if($page === 1 || $countMode==='all'){
                    $sql = "SELECT Count(*) AS totalcount FROM `test_staff_project`   " . (!empty($criteria) ? 'WHERE ' . substr($criteria, 4) : '');
				...
				LEFT JOIN `test_staff` `test_staff1001` ON 
                    `test_staff_project`.`staff_id` = `test_staff1001`.`id`";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
                        $sql = "SELECT Count(*) AS filteredcount FROM `test_staff_project` $join   $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`test_staff_project`.`' . $record['property'] . '` ' . $direction . ', ';
                    }
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_staff_project`.`id` AS `Id`, `test_staff_project`.`date_from` AS `Date From`, `test_staff_project`.`notes` AS `Notes`, `test_staff_project`.`perc_complete` AS `Perc Complete` 
                   , CONCAT(`test_staff1000`.`first_name`, ' ', `test_staff1000`.`last_name`) as `Staff`
				, `test_staff1001`.`mobile` as `Staff Mobile No (parent field)` 
            FROM `test_staff_project` 
                 /*_dib staff_id1 (7192) dib_*/
				LEFT JOIN `test_staff` `test_staff1000` ON 
                    `test_staff_project`.`staff_id` = `test_staff1000`.`id`
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
            ...
            // Check Validation
	        // perc_complete (integer)
            if(isset($attributes['perc_complete']) && trim((string)($attributes['perc_complete'])) !== '') {
                if(!is_int((int)$attributes["perc_complete"]) || !ctype_digit((string)abs((int)$attributes["perc_complete"])))
                    return array ('error',"The 'Perc Complete' field must be an integer value.");
                if($attributes["perc_complete"] < 0)
                    return array ('error',"The 'Perc Complete' field value must be greater than or equal to 0");
                if($attributes["perc_complete"] > 100)
                    return array ('error',"The 'Perc Complete' field value must be less than or equal to 100");
            }               
	        // notes (plain text)
            if(isset($attributes['notes']) && trim((string)($attributes['notes'])) !== '') {
                if(strlen($attributes["notes"]) < 5)
                    return array ('error',"The 'Notes' field cannot contain less than 5 characters");
                if(strlen($attributes["notes"]) > 50)
                    return array ('error',"The 'Notes' field cannot contain more than 50 characters");
            }               
	        // date_from (date)
            if(isset($attributes['date_from']) && trim((string)($attributes['date_from'])) !== '') {
                if(!strtotime(str_replace('/', '-', $attributes["date_from"])))
                    return array ('error',"The 'Date From' field must be a valid date.");
                else
                    $attributes["date_from"] = date('Y-m-d', strtotime(str_replace('/', '-', $attributes["date_from"])));
            }  else
                return array ('error',"The 'Date From' field is required. Please provide a value.");
	        // staff_id (integer)
            if(isset($attributes['staff_id']) && trim((string)($attributes['staff_id'])) !== '') {
                if(!is_int((int)$attributes["staff_id"]) || !ctype_digit((string)abs((int)$attributes["staff_id"])))
                    return array ('error',"The 'Staff' field must be an integer value.");
                if($attributes["staff_id"] < -2147483647)
                    return array ('error',"The 'Staff' field value must be greater than or equal to -2147483647");
                if($attributes["staff_id"] > 2147483647)
                    return array ('error',"The 'Staff' field value must be less than or equal to 2147483647");
            }  else
                return array ('error',"The 'Staff' field is required. Please provide a value.");
	        // project_id (integer)
            if(isset($attributes['project_id']) && trim((string)($attributes['project_id'])) !== '') {
                if(!is_int((int)$attributes["project_id"]) || !ctype_digit((string)abs((int)$attributes["project_id"])))
                    return array ('error',"The 'Project Id' field must be an integer value.");
                if($attributes["project_id"] < -2147483647)
                    return array ('error',"The 'Project Id' field value must be greater than or equal to -2147483647");
                if($attributes["project_id"] > 2147483647)
                    return array ('error',"The 'Project Id' field value must be less than or equal to 2147483647");
            }  else
                return array ('error',"The 'Project Id' field is required. Please provide a value.");
            // All clear - perform the insert...
            $sql = "INSERT INTO `test_staff_project` (";
            $fieldList = '';
            $valueList = '';
            $fieldType = array();
            $i=0;
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue) {
                    ...
                    $this->auditInsert("create", $fieldName, null, $newValue, 71, $recordId, $newDisplayValue, $entityId);
                }
    ...
    public function update($pkValues, $attributes, $clientData=array()) {
        ...
        try {    
            ...
            if(isset($attributes['project_id']) && trim((string)($attributes['project_id'])) !== '') {
                ...
                return array ('error',"The 'Project Id' field is required. Please provide a value.");
            // Staff_MobileNo (time)
            if(isset($attributes['Staff_MobileNo']) && trim((string)($attributes['Staff_MobileNo'])) !== '') {
                if(!strtotime('2000-01-01 ' . (string)$attributes["Staff_MobileNo"]))
                    return array ('error',"The 'Staff Mobile No (parent field)' field must be a valid time value.");
                else
                    $attributes["Staff_MobileNo"] = date('H:i:s', strtotime('2000-01-01 ' . (string)$attributes["Staff_MobileNo"]));
            }  else
                return array ('error',"The 'Staff Mobile No (parent field)' field is required. Please provide a value.");
            // staff_id (integer)
            if(isset($attributes['staff_id']) && trim((string)($attributes['staff_id'])) !== '') {
                if(!is_int((int)$attributes["staff_id"]) || !ctype_digit((string)abs((int)$attributes["staff_id"])))
                    return array ('error',"The 'Staff' field must be an integer value.");
            ...
            $params[":pk1"] = $pkValues["id"];
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            $pkCrit = "`test_staff_project`.`id` = :pk1";
            // Check Unique Values for table option PRIMARY(476)
            $criteria = '1=1 ';           
            if(!array_key_exists('id', $attributes)) {
                $uRst = dibMySqlPdo::execute("SELECT `id` FROM `test_staff_project` WHERE $pkCrit", $this->dbIndex, $params, true);
                $attributes['id'] = $uRst['id'];
            }
            $criteria .= " AND `id` = :fk1 ";
            $criteria .= "AND `id` <> :pk1";
            $sql = "SELECT `test_staff_project`.`id` AS pkv FROM `test_staff_project` WHERE $criteria";
            ...
            $sql = "UPDATE `test_staff_project`  SET ";
            $i=0;
            foreach ($validAttributes AS $key => $value) {
                if(!array_key_exists($key, $recordOld) || (string)$value === (string)$recordOld[$key] || !array_key_exists($key, $this->storeType) || $this->storeType[$key] === 'dibsqli')
                    ...
                    // Only update the fields that have changed values
                        $sql .= "`test_staff_project`.`$key`=:f$i, ";
                    $params[':f'.$i] = $value;
                    $fieldType[':f'.$i] = $this->fieldType[$key];
                    $i++;
                }
            ...
            // Update fields in parent table test_staff
			if(!is_null($attributes['staff_id']) && trim((string)$attributes['staff_id'])!=='') { // Beware... the PHP empty function catches 0,'0',false,etc.       
	            $sql = "UPDATE `test_staff` SET 
	            		`mobile` = :mobile WHERE id = :id";
		        // Set parameter values
		        $updatePar = array();
		        $updatePar[':mobile'] = $attributes['Staff_MobileNo'];
		        // Check if value has changed
		        if($attributes['Staff_MobileNo']!==$recordOld['Staff_MobileNo'])
		        	$parentFieldChanged = true;
		        $updatePar[':id'] = $attributes['staff_id'];
	            $parentSuccess = dibMySqlPdo::execute($sql, $this->dbIndex, $updatePar);
	            if ($parentSuccess === FALSE) 
	            	return array('error',"Could not update the 'test_staff' parent table fields. Please contact the System Administrator.");
            }
                    ...
                    if ($crit===TRUE) {
                        ...
                        foreach ($validAttributes AS $fieldName => $newValue) {
                            if ($newValue !== $recordOld[$fieldName]) {
                                ...
                                $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 71, $recordId, $newDisplayValue, $entityId, $oldDisplayValue);
                            }
                        }
                    } elseif (is_array($crit)) return $crit;
                }
    ...
    public function delete($pkValues, $clientData=array()) {
        try {
            ...
            $sql = "DELETE `test_staff_project` FROM `test_staff_project`   WHERE $crit";
            dibMySqlPdo::setParamsType($fieldType, $this->dbIndex);
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) {
                        ...
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 71, $recordId, null, $entityId, $oldDisplayValue);
                    }
                } elseif (is_array($crit)) return $crit;
                return true;
            }
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `staff_id`, `project_id`, `date_from`, `notes`, `perc_complete`
                    FROM `test_staff_project` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexValidationServerSide.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexValidationServerSide records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexValidationServerSide' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexValidationServerSide' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = "UPDATE test_staff_project SET `" . $args[0] . "` = :value WHERE $pkCrit";					
					DibApp::$array['DuplicateRecords']['test_staff_project*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 311, 'test_staff_project', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Staff", "Notes", "Perc Complete", "Date From", "Project Id" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexValidationServerSide', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexValidationServerSide",
					 'selectFields' => "`test_staff_project`.`id`,`test_staff_project`.`staff_id`,`test_staff_project`.`project_id`,`test_staff_project`.`date_from`,`test_staff_project`.`notes`,`test_staff_project`.`perc_complete`",
				     'selectSqlFields' => trim("
            ", ", \r\n"),
				     'selectSqlDisplay' =>  trim("
            ", ", \r\n"),
				     'selectTableDisplay' => trim(", CONCAT(`test_staff1000`.`first_name`, ' ', `test_staff1000`.`last_name`) as `staff_id_display_value`
				, `test_staff1001`.`mobile` as `Staff_MobileNo`", ", \r\n"),          
                     'from' => trim("`test_staff_project` /*_dib DROPDOWNS - SQL DISPLAY EXPR dib_*/ 
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib staff_id1 (7192) dib_*/
				LEFT JOIN `test_staff` `test_staff1000` ON 
                    `test_staff_project`.`staff_id` = `test_staff1000`.`id`
				/*_dib Staff_MobileNo (7190) dib_*/
				LEFT JOIN `test_staff` `test_staff1001` ON 
                    `test_staff_project`.`staff_id` = `test_staff1001`.`id` /*_dib EXTRA-TABLE-JOIN dib_*/ ", ", \r\n"),
                     'order_by' => "",
                     'filter' => ""
        );
	}
