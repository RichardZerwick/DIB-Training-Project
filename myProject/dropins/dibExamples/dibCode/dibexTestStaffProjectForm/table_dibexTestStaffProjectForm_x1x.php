<?php 
    Class table_dibexTestStaffProjectForm_x1x {
    public $count = 0;
    ...
    public $fieldType = array("id"=>PDO::PARAM_INT, "staff_id"=>PDO::PARAM_INT, "project_id"=>PDO::PARAM_INT, "date_from"=>PDO::PARAM_STR, "date_to"=>PDO::PARAM_STR, "color"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR);
    // List of component store types of fields from only this table, else declare them as dibsqli fields
    public $storeType = array("id"=>'none', "staff_id"=>'dropdown', "project_id"=>'dropdown', "date_from"=>'none', "date_to"=>'none', "color"=>'dibsqli', "notes"=>'none', "updated"=>'none');
    // List of items based on 'sql insert' expresions
    public $sqlFields = array('color'=>"`test_staff1002`.`color`");
    public $fkeyDisplay = array('staff_id'=>"CONCAT(`test_staff1000`.`first_name`, ' ', `test_staff1000`.`last_name`)", 'project_id'=>"`test_project1001`.`name`",
            );
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if($criteria==='') {
            Log::err("The read request for container dibexTestStaffProjectForm specified an active filter by name that could not be resolved in the generated crud code.\r\nTab or container names were probably updated but code was not regenerated. Please regenerate code and try again.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        ...
        if(!empty($activeFilter)) {
            ...
            $fromClause = "`test_staff_project`
                /*_dib staff_id (4211) dib_*/
				LEFT JOIN `test_staff` `test_staff1000` ON 
                    `test_staff_project`.`staff_id` = `test_staff1000`.`id`
				/*_dib project_id (4212) dib_*/
				LEFT JOIN `test_project` `test_project1001` ON 
                    `test_staff_project`.`project_id` = `test_project1001`.`id`
				/*_dib color (6836) dib_*/
				LEFT JOIN `test_staff` `test_staff1002` ON 
                    `test_staff_project`.`staff_id` = `test_staff1002`.`id` 
                ";
        } else  
            $fromClause = "`test_staff_project`  ";
        $criteria .= $permsCrit;
    ...
    public function read ($page=1, $page_size=20, $order=array(), $gridFilter=array(), $filterParams=array(), $activeFilter=null, $phpFilter=null, $phpFilterParams=array(), $group=null, $readType='gridlist', $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_staff_project`.`id`,`test_staff_project`.`staff_id`,`test_staff_project`.`project_id`,`test_staff_project`.`date_from`,`test_staff_project`.`date_to`,`test_staff_project`.`notes`,`test_staff_project`.`updated` 
                               , CONCAT(`test_staff1000`.`first_name`, ' ', `test_staff1000`.`last_name`) as `staff_id_display_value`
				, `test_project1001`.`name` as `project_id_display_value`
				, `test_staff1002`.`color` as `color`
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_staff_project`.`id` AS `Id`, `test_staff_project`.`date_from` AS `Date From`, `test_staff_project`.`date_to` AS `Date To`, `test_staff_project`.`notes` AS `Notes`, `test_staff_project`.`updated` AS `Updated` 
                   , CONCAT(`test_staff1000`.`first_name`, ' ', `test_staff1000`.`last_name`) as `Staff`
				, `test_project1001`.`name` as `Project`
				, `test_staff1002`.`color` as `Staff Color` 
            FROM `test_staff_project` 
                 /*_dib staff_id (4211) dib_*/
				LEFT JOIN `test_staff` `test_staff1000` ON 
                    `test_staff_project`.`staff_id` = `test_staff1000`.`id`
...
$sql .= $criteria . " " . $orderStr . $limit; 
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL, $clientData=array()) {
        ...
        try { 
	        ...
	        // date_to (date)
            if(isset($attributes['date_to']) && trim((string)($attributes['date_to'])) !== '') {
                if(!strtotime(str_replace('/', '-', $attributes["date_to"])))
                    return array ('error',"The 'Date To' field must be a valid date.");
                else
                    $attributes["date_to"] = date('Y-m-d', strtotime(str_replace('/', '-', $attributes["date_to"])));
            }               
	        // staff_id (integer)
            ...
            if(isset($attributes['project_id']) && trim((string)($attributes['project_id'])) !== '') {
                if(!is_int((int)$attributes["project_id"]) || !ctype_digit((string)abs((int)$attributes["project_id"])))
                    return array ('error',"The 'Project' field must be an integer value.");
                if($attributes["project_id"] < -2147483647)
                    return array ('error',"The 'Project' field value must be greater than or equal to -2147483647");
                if($attributes["project_id"] > 2147483647)
                    return array ('error',"The 'Project' field value must be less than or equal to 2147483647");
            }  else
                return array ('error',"The 'Project' field is required. Please provide a value.");
	        // updated (datetime)
        ...
        try {    
            ...
            if(isset($attributes['color']) && trim((string)($attributes['color'])) !== '') {
                if(strlen($attributes["color"]) > 50)
                    return array ('error',"The 'Staff Color' field cannot contain more than 50 characters");
            }               
			...
			if(!is_null($attributes['staff_id']) && trim((string)$attributes['staff_id'])!=='') { // Beware... the PHP empty function catches 0,'0',false,etc.       
		        ...
		        $updatePar = array();
		        $updatePar[':color'] = $attributes['color'];
		        // Check if value has changed
		        if($attributes['color']!==$recordOld['color'])
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `staff_id`, `project_id`, `date_from`, `date_to`, `notes`, `updated`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err("SQL error while fetching values to duplicate for dibexTestStaffProjectForm.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexTestStaffProjectForm records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexTestStaffProjectForm' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexTestStaffProjectForm' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId, $newDisplayValue, $entityId, $oldDisplayValue=null) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 168, 'test_staff_project', :recordId, '" . $this->now . "', '" . $this->ipAddress . "', '$fieldName', :oldValue, :newValue, :newDisplayValue, :oldDisplayValue, :entityId, :entity_field_name, " . DIB::$USER['id'] . ", :username, " . $this->unique_record . ')';
    ...
    public function getCaptions() {
    	return array("Id", "Staff", "Project", "Date From", "Date To", "Notes", "Updated" );
    }
    private function resolvePhpParam($sql, &$params, &$filterParams) {
        ...
        while($i !== FALSE && $counter < 11) {
            ...
            if($j !== FALSE) {
                ...
                $callResult = $className::$functionName('dibexTestStaffProjectForm', $params, $filterParams, $sql, $var);
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexTestStaffProjectForm",
					 'selectFields' => "`test_staff_project`.`id`,`test_staff_project`.`staff_id`,`test_staff_project`.`project_id`,`test_staff_project`.`date_from`,`test_staff_project`.`date_to`,`test_staff_project`.`notes`,`test_staff_project`.`updated`",
            ...
            ", ", \r\n"),
             ...
             /*_dib DROPDOWNS - TABLE DISPLAY EXPR dib_*/ /*_dib staff_id (4211) dib_*/
				LEFT JOIN `test_staff` `test_staff1000` ON 
                    `test_staff_project`.`staff_id` = `test_staff1000`.`id`
				/*_dib project_id (4212) dib_*/
				LEFT JOIN `test_project` `test_project1001` ON 
				...
				/*_dib color (6836) dib_*/
				LEFT JOIN `test_staff` `test_staff1002` ON 
                    `test_staff_project`.`staff_id` = `test_staff1002`.`id` /*_dib EXTRA-TABLE-JOIN dib_*/ ", ", \r\n"),
                     'order_by' => "",
                     'filter' => ""
        );
	}
